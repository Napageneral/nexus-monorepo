...
func health(ctx context.Context, account string) (*nexadapter.AdapterHealth, error) {
	...
	healthPath := firstNonBlank(strings.TrimSpace(os.Getenv("NEXUS_PATIENT_NOW_HEALTH_PATH")), defaultHealthPath)
	params := url.Values{}
...
func backfill(ctx context.Context, account string, since time.Time, emit nexadapter.EmitFunc) error {
	...
	if strings.TrimSpace(creds.BaseURL) == "" {
		return errors.New("patient now API base_url is not configured; use CSV upload until partner API access is available")
	}

	metricsPath := firstNonBlank(strings.TrimSpace(os.Getenv("NEXUS_PATIENT_NOW_METRICS_PATH")), defaultMetricsPath)
	params := url.Values{}
	params.Set("practice_id", creds.PracticeID)
	...
	for _, point := range metrics {
		...
		for _, event := range buildPatientNowEvents(creds, point) {
			emit(event)
		}
	}
...
func buildPatientNowEvents(creds patientNowCredentials, point metricPoint) []nexadapter.NexusEvent {
	...
	eventID := strings.Join(
		[]string{
			...
			sanitizeToken(creds.PracticeID),
...
func resolvePatientNowCredentials(account string) (patientNowCredentials, error) {
	...
	if err == nil && runtimeContext != nil {
		...
		if runtimeContext.Credential != nil {
			...
			practiceID = firstNonBlank(
				fieldValue(fields, "practice_id"),
				fieldValue(fields, "practice"),
			)
