...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--workspace-db") {
      opts.workspaceDbPath = text(argv[index + 1]);
      index += 1;
      continue;
    }
    ...
    if (arg === "--help" || arg === "-h") {
      process.stdout.write(
        [
          ...
          "",
          "Options:",
          "  --workspace-db <path>      Path to frontdoor workspace sqlite db",
          ...
          "  workspace db: --workspace-db, FRONTDOOR_WORKSPACE_STORE_PATH, config.workspace.storePath,",
          "                fallback near session store as frontdoor-workspaces.db",
...
function resolveWorkspaceDbPath(options, configPath, config) {
  const fromArg = text(options.workspaceDbPath);
  if (fromArg) {
    return path.resolve(fromArg);
  }
  const fromEnv = text(process.env.FRONTDOOR_WORKSPACE_STORE_PATH);
  if (fromEnv) {
    return path.resolve(fromEnv);
  }
  const configDir = path.dirname(configPath);
  const fromConfig = text(config?.workspace?.storePath);
  if (fromConfig) {
  ...
  if (sessionStorePath) {
    ...
    return path.resolve(path.dirname(resolved), "frontdoor-workspaces.db");
  }
  return path.resolve(configDir, "frontdoor-workspaces.db");
}
...
function main() {
  ...
  const workspaceDbPath = resolveWorkspaceDbPath(options, configPath, config);
