...
5. `app`, `adapter`, and `service` are different package kinds and must not be blurred together.
...
| `app` | Installable product package launched through frontdoor |
| `adapter` | Shared installable integration package |
| `service` | Shared installable backend package or app-local process, depending on context |
| `package` | Generic registry unit; can be an app, adapter, or shared service |
