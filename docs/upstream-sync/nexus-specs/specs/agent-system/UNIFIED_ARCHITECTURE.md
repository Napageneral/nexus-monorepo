...
1. **Mnemonic Event Layer** — Universal event sourcing, storage, and hook evaluation
...
│  │  • Uses Mnemonic search to find best target                     │        │
...
│  │  • Mnemonic context injection (TODO: spec details)      │        │
│  └─────────────────────────────────────────────────────────┘        │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────┐        │
│  │                    QUEUE MANAGEMENT                      │        │
...
│  2. Store response as Mnemonic event (closes the loop)                      │
...
interface RoutingTarget {
  ...
  // Modifier:
  smart?: boolean;    // Use Mnemonic to find best target first
}
...
1. Query Mnemonic for best matching thread/session/persona
...
External Source (e.g., iMessage)
        │
        ▼
Inbound Adapter normalizes → Mnemonic Event
        │
        ▼
...
Agent execution (pi-agent)
        │
        ▼
Response captured → stored as Mnemonic event
        │
        ▼
...
Mnemonic picks up all agent sessions via AIX sync (for search/analysis), but real-time routing is broker-direct.
...
### Memory System
- `cortex/docs/MNEMONIC_ARCHITECTURE.md` — Unified memory system (Events + Agents ledgers)
- `aix/docs/AIX_MNEMONIC_PIPELINE.md` — AIX capture → Mnemonic flow
