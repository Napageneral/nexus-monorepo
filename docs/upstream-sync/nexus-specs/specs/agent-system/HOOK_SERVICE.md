...
This document specifies the **Hook Service** — the component responsible for hook registration, lifecycle management, and event evaluation. Rather than a file-watching daemon, hooks are registered via CLI, giving agents explicit control and enabling rich metadata capture.

The Hook Service is part of a **unified Nexus service** that handles:
- Event ingestion (Mnemonic adapters)
- Hook evaluation
- Agent Broker dispatch
- Session management
...
CREATE TABLE hook_invocations (
  ...
  event_id TEXT NOT NULL,           -- FK to Mnemonic events
...
│  │                    EVENT INGESTION                                   │    │
│  │                                                                      │    │
│  │  Mnemonic Adapters → Normalize → Store in events table              │    │
...
Event arrives (iMessage, timer tick, webhook, etc.)
        │
        ▼
Store in Mnemonic events table
        │
        ▼
...
| Question | Where |
|----------|-------|
| How does service receive events? | Mnemonic adapter spec |
...
*This spec captures the Hook Service design. It should be unified with ongoing Broker and Mnemonic specs as those evolve.*
