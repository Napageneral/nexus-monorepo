...
interface RoutingHookContext {
  event: MnemonicEvent;        // The incoming event
  mnemonic: MnemonicClient;    // For querying contacts, history
  now: Date;
}
...
export default async function(ctx: RoutingHookContext): Promise<RoutingHookResult> {
  ...
  // Option 2: Query mnemonic contacts (preferred)
  // const owner = await mnemonic.query({ 
  //   table: 'people', 
  //   where: { is_user: true } 
  // });
...
export default async function(ctx: RoutingHookContext): Promise<RoutingHookResult> {
  ...
  // Could also query: await mnemonic.query({ table: 'people', where: { relationship: 'family' } })
...
## Integration with Mnemonic Contacts

For cleaner identity management, hooks can query Mnemonic's people/contacts tables:
...
const family = await mnemonic.query({
  table: 'people', 
...
const person = await mnemonic.findPersonByIdentity(senderId);
