...
| Discovery | Via config | Via `nexus skill list` |
...
3. User installs packs: `nexus skill install pack:macos-essentials`
...
nexus skill list --type tools       # Filter by type
nexus skill use <name>              # Get skill guide
nexus skill info <name>             # Skill metadata + status
nexus skill install <pack|name>     # Install from hub
nexus skill update                  # Update managed skills
```
