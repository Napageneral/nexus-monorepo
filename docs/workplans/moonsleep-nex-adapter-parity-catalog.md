...
Production bedrock readback on 2026-07-21: the fresh MoonSleep Nex service is enabled and healthy on the Ops VPS at release `c43eeb790b79e2aca0044d9c499223a7a057e80b`. It contains exactly the fresh entities `Casey`, `MoonSleep Ops`, and `Tyler`, zero contacts, zero personal-state markers, zero adapters, zero business records/events/search documents, and zero job definitions/subscriptions/schedules/runs/queue rows. PostgreSQL owns records, durable events, search projection, identity, subscriptions, jobs, queueing, dispatch receipts, and idempotency; the seven canonical work lanes exist and the corresponding SQLite business planes remain empty. Backup/restore, restart, signal cleanup, public identity/work operations, private Tailscale access, and token-safe local operator reads passed. No adapter lane may treat this empty deployment as provider admission or cutover authority.
...
- **Completion:** install the adapter and app dormant, prove a bounded customer cohort and an idempotent two-pass full backfill, project customer/order state, then activate PostgreSQL-native event dispatch and reconcile full history and incrementals while preserving every Dispatch/Shopify action with readback. The customer job and subscription remain deliberately dormant until cohort, backfill, restart, replay, and rollback proof close the activation gate.
- **Approved source-job topology:** independent Nex jobs execute in separate processes and never share a mutable in-process cursor. `orders.delta`, `customers.delta`, and `inventory.hot` are designed for one-minute observation; `inventory.reconcile`, `fulfillment.delta`, `discounts.delta`, `finance.transactions`, and `disputes.delta` for five-minute observation; `products.delta` and `catalog.delta` for fifteen-minute observation; `marketing.delta` hourly; `payouts.delta` every six hours. Every family has an independent cursor, single-flight capture lease, commit/abort receipt, disabled-first schedule, and force-now invocation. A store-wide cross-process governor initially permits at most two concurrent provider requests, paces them, and persists shared 429/backpressure state. Webhooks may wake a job earlier but never replace reconciliation pulls.
- **Current-reader parity:** the new observation layer must cover the minute order sync, ten-minute wide order reconciliation, preorder customer/order reads, inventory quota reads, fulfillment tracking reads, and Finance payouts/balance/disputes reads before any legacy provider read is retired. Consequential Shopify mutations remain separate typed domain actions; the source adapter never acquires inventory, tag, tracking, refund, or fulfillment write authority merely because it observes those objects.
- **Activation sequence:** fixtures and Linux/AMD64 PostgreSQL cleanroom first; dormant production install second; one observation family at a time in shadow; compare IDs, counts, revisions, money, nulls, relationships, cursor gaps, provider-call volume, 429s, and freshness; then move consumers one by one. The provider monitor stays disabled until this family-level shadow gate is explicitly passed.
