# AAA-005 Google Ads Full-Surface Provider Method Catalog

## Goal

Push `google-ads` from a projection-only package toward the canonical
...
- `google-ads` currently exposes only adapter runtime operations plus the
  ingest contract
- the package does not yet expose a public `google-ads.*` provider-native
  method surface
...
1. the package has an explicit first public `google-ads.*` method slice
