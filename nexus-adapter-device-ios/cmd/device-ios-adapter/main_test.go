...
func TestSetupSubmitRequiresConfirmation(t *testing.T) {
	pending, err := setupSubmit(context.Background(), nexadapter.AdapterSetupRequest{
		...
		Payload: map[string]any{
			"confirm_companion_installed": "no",
			"confirm_paired":              "no",
		},
	})
	if err != nil {
...
func TestControlStartUpsertsEndpointAndHandlesInvoke(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-1","endpoint_id":"ios-default","command":"camera.snap","payload":{"quality":"high"}}` + "\n")
...
func TestControlStartRejectsUnknownCommand(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-2","endpoint_id":"ios-default","command":"system.run","payload":{}}` + "\n")
	var out strings.Builder
	session := nexadapter.NewControlSession(input, &out)
