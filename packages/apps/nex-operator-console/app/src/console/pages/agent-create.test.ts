...
describe("agent create wizard", () => {
  it("renders step 1 basics form", () => {
    ...
    const nameInput = container.querySelector("input.console-input");
    expect(nameInput).not.toBeNull();

    const labels = Array.from(container.querySelectorAll(".console-label"));
    expect(labels.some((l) => l.textContent?.includes("Name"))).toBe(true);
    expect(labels.some((l) => l.textContent?.includes("Model"))).toBe(true);

    const models = container.querySelectorAll(".console-selectable");
