  ...
  return html`
    ...
    >
      ...
      ${run.episode_count != null
        ? html`<div class="console-faint" style="font-size: var(--console-text-2xs); margin-top: var(--console-space-1);">${run.episode_count} episode${run.episode_count === 1 ? "" : "s"}</div>`
        : nothing}
    </div>
  `;
}
    ...
    >
      ...
      <div style="display: flex; gap: var(--console-space-3); margin-top: var(--console-space-1);">
        ${ep.observation_count != null
          ? html`<span class="console-faint" style="font-size: var(--console-text-2xs);">${ep.observation_count} obs</span>`
          : nothing}
        ${ep.fact_count != null
          ? html`<span class="console-faint" style="font-size: var(--console-text-2xs);">${ep.fact_count} facts</span>`
          : nothing}
        ${ep.started_at != null
  ...
  return html`
    ...
    >
      <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: var(--console-space-2);">
        <div class="console-muted" style="font-size: var(--console-text-sm); flex: 1;">${obs.text}</div>
  ...
  return html`
    ...
    >
      <div style="display: flex; justify-content: space-between; align-items: center; gap: var(--console-space-2);">
        <div style="flex: 1; font-size: var(--console-text-sm);">
          <span class="console-strong">${fact.subject ?? "?"}</span>
          <span class="console-muted" style="margin: 0 var(--console-space-1);">\u2192</span>
          <span class="console-mono" style="font-size: var(--console-text-xs);">${fact.predicate ?? "?"}</span>
          <span class="console-muted" style="margin: 0 var(--console-space-1);">\u2192</span>
          <span class="console-strong">${fact.object ?? "?"}</span>
        </div>
  ...
  return html`
    <div>
      ${detail.summary
        ? html`
            <div class="console-card" style="margin-bottom: var(--console-space-4);">
              <div class="console-section-label" style="margin-bottom: var(--console-space-2);">Summary</div>
              <div class="console-muted" style="font-size: var(--console-text-sm);">${detail.summary}</div>
            </div>
          `
        : nothing}
      ...
      ${observations.length === 0
        ? html`<div class="console-muted" style="font-size: var(--console-text-xs); padding: var(--console-space-3);">No observations extracted.</div>`
      ...
      ${facts.length === 0
        ? html`<div class="console-muted" style="font-size: var(--console-text-xs); padding: var(--console-space-3);">No facts extracted.</div>`
...
function renderSearchResult(
  ...
  return html`
    ...
    >
      ...
      ${result.entity_id
        ? html`<div class="console-faint" style="font-size: var(--console-text-2xs); margin-top: var(--console-space-1);">entity: ${result.entity_id}</div>`
        : nothing}
    </div>
  `;
}
                    ...
                    ${qualityItems.map(
                      (item) => html`
                        ...
                        >
                          ...
                          <td>
                            <div class="console-mono" style="font-size: var(--console-text-2xs); line-height: 1.5;">
...
function renderDetailPanel(props: MemoryPageProps) {
  ...
  if (props.detailLoading) {
    return html`
      <div class="console-card" style="margin-top: var(--console-space-4);">
        ${renderSpinner()}
      </div>
    `;
  }
...
function renderEntityDetail(
  ...
  return html`
    <div class="console-card" style="margin-top: var(--console-space-4); position: relative;">
      <button
      ...
      <div style="display: flex; align-items: center; gap: var(--console-space-2); margin-bottom: var(--console-space-3);">
        ...
        <span class="console-strong" style="font-size: var(--console-text-md);">${entity.name || entity.id}</span>
      </div>
        ...
        ? html`
            ...
            ${observations.map(
              (o) => html`
                ...
                >
                  <span class="console-muted" style="font-size: var(--console-text-sm);">${o.text}</span>
                </div>
              `,
            )}
          `
  ...
  return html`
    <div class="console-card" style="margin-top: var(--console-space-4); position: relative;">
      ...
      <div style="display: flex; align-items: center; gap: var(--console-space-2); margin-bottom: var(--console-space-3);">
        ...
        <span class="console-mono" style="font-size: var(--console-text-xs);">${fact.id}</span>
      </div>

      <div style="display: grid; grid-template-columns: auto 1fr; gap: var(--console-space-2) var(--console-space-4); font-size: var(--console-text-sm);">
        <span class="console-faint">Subject</span>
        <span class="console-strong">${fact.subject ?? "—"}</span>
        <span class="console-faint">Predicate</span>
        <span class="console-mono">${fact.predicate ?? "—"}</span>
        <span class="console-faint">Object</span>
        <span class="console-strong">${fact.object ?? "—"}</span>
        <span class="console-faint">Confidence</span>
        <span>${confidenceBadge(fact.confidence)}</span>
        <span class="console-faint">Source Episode</span>
        <span class="console-mono" style="font-size: var(--console-text-xs);">${fact.source_episode_id ?? "—"}</span>
      </div>
    </div>
  `;
}
  ...
  return html`
    <div class="console-card" style="margin-top: var(--console-space-4); position: relative;">
      ...
      <div style="display: flex; align-items: center; gap: var(--console-space-2); margin-bottom: var(--console-space-3);">
        ...
        <span class="console-mono" style="font-size: var(--console-text-xs);">${obs.id}</span>
      </div>

      <div class="console-muted" style="font-size: var(--console-text-sm); margin-bottom: var(--console-space-3);">
        ${obs.text}
      </div>
      ...
      <div style="display: grid; grid-template-columns: auto 1fr; gap: var(--console-space-2) var(--console-space-4); font-size: var(--console-text-sm);">
        ...
        <span class="console-faint">Source Episode</span>
        <span class="console-mono" style="font-size: var(--console-text-xs);">${obs.source_episode_id ?? "—"}</span>
        ${obs.entity_id
          ? html`
              <span class="console-faint">Entity</span>
              <span
                class="console-mono"
                style="font-size: var(--console-text-xs); cursor: pointer; color: var(--console-accent); text-decoration: underline;"
