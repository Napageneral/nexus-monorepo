...
describe("usage-helpers", () => {
  ...
  it("matches key: glob filters against session keys", () => {
    const session = {
      key: "agent:main:schedule:16234bc?token=dev-token",
      label: "agent:main:schedule:16234bc?token=dev-token",
      usage: { totalTokens: 100, totalCost: 0 },
    };
    const matches = filterSessionsByQuery([session], "key:agent:main:schedule*");
