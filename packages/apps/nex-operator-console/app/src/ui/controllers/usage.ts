...
export async function loadUsage(
  ...
  try {
    ...
    const [sessionsRes, costRes] = await Promise.all([
      state.client.request("agents.sessions.usage", {
        startDate,
        endDate,
...
export async function loadSessionTimeSeries(state: UsageState, sessionKey: string) {
  ...
  try {
    const res = await state.client.request("agents.sessions.usage.timeseries", { key: sessionKey });
    if (res) {
...
export async function loadSessionLogs(state: UsageState, sessionKey: string) {
  ...
  try {
    const res = await state.client.request("agents.sessions.usage.logs", {
      key: sessionKey,
      limit: 500,
    });
