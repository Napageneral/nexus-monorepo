import type { MonitorOperation, MonitorOperationsStatsResult } from "../types.ts";
...
export function normalizeMonitorOperation(raw: RawMonitorOperation): MonitorOperation {
  const phase =
...
export async function loadMonitorHistory(
  ...
  try {
    ...
    state.monitorHistoryError = error instanceof Error ? error.message : String(error);
  } finally {
...
export async function loadMonitorStats(state: MonitorState): Promise<void> {
  ...
  try {
    ...
    // Stats are supplementary; history remains available when this request fails.
  } finally {
