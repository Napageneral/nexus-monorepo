...
describe("identity controller", () => {
  it("loads canonical identity surfaces together", async () => {
    const request = vi.fn(async (method: string) => {
      switch (method) {
        case "entities.list":
          return { entities: [{ id: "entity-1", name: "Operator", type: "person" }] };
        case "contacts.list":
          return { contacts: [{ id: "contact-1", contact_name: "Operator" }] };
        case "channels.list":
  ...
  it("loads entity detail with canonical contacts", async () => {
    const request = vi.fn(async (method: string, params?: unknown) => {
      switch (method) {
        case "entities.get":
          return { entity: { id: "entity-1", name: "Operator", type: "person" } };
        case "contacts.list":
    ...
    expect(state.identitySelectedEntity?.name).toBe("Operator");
  ...
  it("clears selected identity detail", () => {
    ...
    state.identitySelectedEntity = { id: "entity-1", name: "Operator" };
