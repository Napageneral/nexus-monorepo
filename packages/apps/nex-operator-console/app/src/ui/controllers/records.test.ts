  ...
  it("loads record channels through channels.list", async () => {
    const request = vi.fn(async (method: string, params?: unknown) => {
      if (method === "channels.list") {
        ...
        return {
          channels: [{ id: "channel-1", platform: "imessage", thread_name: "Operator" }],
        };
      }
      throw new Error(`unexpected method: ${method}`);
    });
    ...
    expect(state.recordsChannels).toEqual([
      { id: "channel-1", platform: "imessage", thread_name: "Operator" },
    ]);
  ...
  it("searches records only when a query is present", async () => {
    const request = vi.fn(async (method: string, params?: unknown) => {
      if (method === "records.search") {
        expect(params).toEqual({ query: "operator", limit: 50, platform: undefined });
        return {
          records: [{ id: "record-1", record_id: "r1", platform: "imessage", content: "operator" }],
        };
      }
      throw new Error(`unexpected method: ${method}`);
    });
    ...
    const state = createState(request);
    state.recordsSearchQuery = "operator";
    await searchRecords(state);

    expect(state.recordsSearchResults).toEqual([
      { id: "record-1", record_id: "r1", platform: "imessage", content: "operator" },
    ]);
