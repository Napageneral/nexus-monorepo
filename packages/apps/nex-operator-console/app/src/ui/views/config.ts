...
export function renderConfig(props: ConfigProps) {
  ...
  const sectionOrder = new Map(
    ["agents", "auth", "runtime", "tools", "skills", "env", "update"].map((key, index) => [
      key,
      index,
    ]),
  );
  const sortedSections = [...allSections].toSorted((a, b) => {
    const aOrder = sectionOrder.has(a.key) ? (sectionOrder.get(a.key) as number) : 999;
    const bOrder = sectionOrder.has(b.key) ? (sectionOrder.get(b.key) as number) : 999;
    if (aOrder !== bOrder) {
      return aOrder - bOrder;
    }
    return a.label.localeCompare(b.label);
  });
  const legacySectionKeys = new Set(["channels", "messages", "commands", "hooks", "wizard"]);
  const primarySections = sortedSections.filter((section) => !legacySectionKeys.has(section.key));
  const legacySections = sortedSections.filter((section) => legacySectionKeys.has(section.key));
  const navSections = props.searchQuery ? sortedSections : primarySections;
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        ...
        <nav class="config-nav">
          ...
          ${
            !props.searchQuery && legacySections.length > 0
              ? html`
                  <details class="config-nav-legacy">
                    <summary class="config-nav-legacy__summary">Legacy</summary>
                    ${legacySections.map(
                      (section) => html`
                        <button
                          class="config-nav__item ${props.activeSection === section.key ? "active" : ""}"
