...
import type { AdapterConnectionEntry } from "../controllers/integrations.ts";
import type {
  AgentFileEntry,
...
export type AgentsPanel = "overview" | "files" | "tools" | "skills" | "accounts" | "automations";
...
export type AgentsProps = {
  ...
  configDirty: boolean;
  adapterConnectionsLoading: boolean;
  adapterConnectionsError: string | null;
...
const TOOL_SECTIONS = [
  ...
  },
  {
    ...
    label: "Web",
    tools: [
      { id: "nex.web.search", label: "nex.web.search", description: "Search the web" },
      { id: "nex.web.fetch", label: "nex.web.fetch", description: "Fetch web content" },
    ],
  },
  {
    id: "memory",
    label: "Memory",
    tools: [
      { id: "nex.memory.recall", label: "nex.memory.recall", description: "Recall relevant memory and prior context" },
    ],
  },
  {
    id: "sessions",
    label: "Sessions",
    tools: [
      { id: "nex.sessions.list", label: "nex.sessions.list", description: "List sessions" },
      { id: "nex.sessions.history", label: "nex.sessions.history", description: "Session history" },
      { id: "nex.agent.dispatch", label: "nex.agent.dispatch", description: "Dispatch work to another agent session" },
      { id: "nex.agent.wait", label: "nex.agent.wait", description: "Wait for worker progress or completion" },
      { id: "nex.session.status", label: "nex.session.status", description: "Session status" },
    ],
  },
  {
    id: "ui",
  ...
  },
  {
    ...
    label: "Automation",
    tools: [
      { id: "schedule", label: "schedule", description: "Schedule tasks" },
      { id: "runtime", label: "runtime", description: "Runtime control" },
    ],
  },
  {
    id: "nodes",
    label: "Nodes",
    tools: [{ id: "nodes", label: "nodes", description: "Nodes" }],
  },
  {
    id: "agents",
    label: "Agents",
              ...
              ${
                props.activePanel === "accounts"
                  ? renderAgentChannels({
                      ...
                      onRefresh: props.onAccountsRefresh,
                    })
                  : nothing
              }
              ${
                props.activePanel === "automations"
                  ? renderAgentSchedule({
                      ...
                      onRefresh: props.onScheduleRefresh,
                    })
                  : nothing
              }
            `
...
function renderAgentTabs(active: AgentsPanel, onSelect: (panel: AgentsPanel) => void) {
  const tabs: Array<{ id: AgentsPanel; label: string }> = [
    ...
    { id: "accounts", label: "Linked Integrations" },
    { id: "automations", label: "Related Operations" },
  ];
  return html`
  ...
  return html`
    <section class="grid grid-cols-2">
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">Linked Integrations</div>
            <div class="card-sub">Integrations, connections, and account ownership visible to this runtime.</div>
          </div>
        ...
        <div class="row" style="gap: 8px; margin-top: 10px; flex-wrap: wrap;">
          <span class="pill">Adapters: ${entries.length}</span>
          <span class="pill ok">Connected: ${connected}</span>
        ...
        ${
          entries.length === 0
            ? html`
                <div class="muted" style="margin-top: 16px">No adapters found.</div>
              `
            : html`
                  ...
                  return html`
                    <div class="list-item">
                      <div class="list-main">
                        ...
                        <div class="list-sub mono">${entry.adapter}</div>
                      </div>
                      <div class="list-meta">
                        <div><span class="pill ${statusClass}">${entry.status}</span></div>
                        <div>account: <span class="mono">${entry.account ?? "—"}</span></div>
                        <div>
                          last sync:
                          ${entry.lastSync ? formatRelativeTimestamp(entry.lastSync) : "n/a"}
                        </div>
...
function renderAgentSchedule(params: {
  agent: AgentsListResult["agents"][number];
  defaultId: string | null;
  configForm: Record<string, unknown> | null;
  agentFilesList: AgentsFilesListResult | null;
  ...
  const recentJobs = params.jobs
    .toSorted((a, b) => {
      const left = Date.parse(a.next_run_at ?? a.updated_at);
      const right = Date.parse(b.next_run_at ?? b.updated_at);
  ...
  return html`
    <section class="grid grid-cols-2">
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">Related Operations</div>
            <div class="card-sub">Runtime schedule and execution context touching this agent.</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? "Refreshing…" : "Refresh"}
          </button>
    ...
    <section class="card">
      ...
      <div class="card-sub">Schedules are runtime-owned job bindings, not agent-owned records.</div>
      <div class="muted" style="margin-top: 12px">
        Manage schedule definitions from Operations. This agent panel only shows the current
        runtime schedule surface around this agent.
      </div>
      ${
        recentJobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">No schedules configured.</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${recentJobs.map(
                  ...
                  <div class="list-item">
                    <div class="list-main">
                      ...
                      <div class="chip-row" style="margin-top: 6px;">
                        ...
                        <span class="chip">${job.job_definition_id}</span>
                      </div>
                    </div>
                    <div class="list-meta">
                      <div class="mono">${formatScheduleState(job)}</div>
                      <div class="muted">${formatSchedulePayload(job)}</div>
                    </div>
                  </div>
                `,
                )}
    ...
    <section class="card">
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                Load the runtime config to adjust tool profiles.
              </div>
            `
          : nothing
      }
...
const SKILL_SOURCE_GROUPS: Array<{ id: string; label: string; sources: string[] }> = [
  { id: "workspace", label: "Workspace Skills", sources: ["nexus-workspace"] },
  { id: "built-in", label: "Built-in Skills", sources: ["nexus-bundled"] },
  { id: "installed", label: "Installed Skills", sources: ["nexus-managed"] },
  { id: "extra", label: "Extra Skills", sources: ["nexus-extra"] },
];
    ...
    <section class="card">
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                Load the runtime config to set per-agent skills.
              </div>
            `
          : nothing
      }
