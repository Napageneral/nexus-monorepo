...
export function handleAgentEvent(host: ToolStreamHost, payload?: AgentEventPayload) {
  ...
  const matchesCurrentRun = Boolean(host.chatRunId && payload.runId === host.chatRunId);
  const matchesCurrentConversation = sessionBelongsToConversation(
    host.sessionsResult,
    host.conversationId,
    sessionKey,
  );
  if (!matchesCurrentRun && sessionKey && !matchesCurrentConversation) {
    return;
  }
  if (!matchesCurrentRun && !sessionKey) {
    return;
  }
  if (host.chatRunId && payload.runId !== host.chatRunId && !matchesCurrentConversation) {
    return;
  }
  if (!host.chatRunId) {
    return;
