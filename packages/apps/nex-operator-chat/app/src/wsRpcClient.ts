...
import {
  createNexEmbeddedWsRpcClient,
  resetNexEmbeddedWsRpcClient,
} from "./nex/ws-rpc-client";
...
let sharedWsRpcClientMode: "embedded" | "standalone" | null = null;

export function getWsRpcClient(): WsRpcClient {
  const mode = isNexEmbedded() ? "embedded" : "standalone";
  if (sharedWsRpcClient && sharedWsRpcClientMode === mode) {
    return sharedWsRpcClient;
  }

  if (sharedWsRpcClient) {
    void sharedWsRpcClient.dispose().catch(() => undefined);
  }

  sharedWsRpcClient =
    mode === "embedded" ? createNexEmbeddedWsRpcClient() : createWsRpcClient();
  sharedWsRpcClientMode = mode;
  return sharedWsRpcClient;
}
