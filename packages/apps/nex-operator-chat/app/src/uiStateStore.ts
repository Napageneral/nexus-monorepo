...
function defaultProjectExpanded(): boolean {
  return isNexEmbedded() ? false : true;
}
...
export function toggleProject(state: UiState, projectId: ProjectId): UiState {
  const expanded = state.projectExpandedById[projectId] ?? defaultProjectExpanded();
  return {
    ...state,
    projectExpandedById: {
...
export function setProjectExpanded(
  ...
  if ((state.projectExpandedById[projectId] ?? defaultProjectExpanded()) === expanded) {
    return state;
  }
  return {
    ...state,
