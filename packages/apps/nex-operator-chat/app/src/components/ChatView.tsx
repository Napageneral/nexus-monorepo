...
const LazyBranchToolbar = lazy(() => import("./BranchToolbar"));
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const threadCreationEnabled = isNexFeatureEnabled("threadCreation");
  const gitFeatureEnabled = isNexFeatureEnabled("git");
  const diffFeatureEnabled = isNexFeatureEnabled("diff");
  const terminalFeatureEnabled = isNexFeatureEnabled("terminal");
  ...
  const canCheckoutPullRequestIntoThread = gitFeatureEnabled && isLocalDraftThread;
  const diffOpen = diffFeatureEnabled && rawSearch.diff === "1";
  ...
  // Nex chat is a durable session ledger, so historical tool/work rows must
  // remain visible after a newer turn starts.
  ...
  const terminalToggleShortcutLabel = useMemo(
    () => (terminalFeatureEnabled ? shortcutLabelForCommand(keybindings, "terminal.toggle") : null),
    [keybindings, terminalFeatureEnabled],
  );
  const splitTerminalShortcutLabel = useMemo(
    () =>
      terminalFeatureEnabled
        ...
        : null,
    [keybindings, terminalFeatureEnabled, terminalShortcutLabelOptions],
  );
  ...
  const diffPanelShortcutLabel = useMemo(
    () =>
      diffFeatureEnabled
        ...
        : null,
    [diffFeatureEnabled, keybindings, nonTerminalShortcutLabelOptions],
  );
  ...
  }, [diffFeatureEnabled, diffOpen, navigate, threadId]);
  ...
  const toggleTerminalVisibility = useCallback(() => {
    if (!terminalFeatureEnabled || !activeThreadId) return;
    setTerminalOpen(!terminalState.terminalOpen);
  }, [activeThreadId, setTerminalOpen, terminalFeatureEnabled, terminalState.terminalOpen]);
  const splitTerminal = useCallback(() => {
    if (!terminalFeatureEnabled || !activeThreadId || hasReachedSplitLimit) return;
    const terminalId = `terminal-${randomUUID()}`;
    storeSplitTerminal(activeThreadId, terminalId);
    setTerminalFocusRequestId((value) => value + 1);
  }, [activeThreadId, hasReachedSplitLimit, storeSplitTerminal, terminalFeatureEnabled]);
  const createNewTerminal = useCallback(() => {
    if (!terminalFeatureEnabled || !activeThreadId) return;
    const terminalId = `terminal-${randomUUID()}`;
    storeNewTerminal(activeThreadId, terminalId);
    setTerminalFocusRequestId((value) => value + 1);
  }, [activeThreadId, storeNewTerminal, terminalFeatureEnabled]);
  const closeTerminal = useCallback(
    (terminalId: string) => {
      const api = readNativeApi();
      if (!terminalFeatureEnabled || !activeThreadId || !api) return;
      const isFinalTerminal = terminalState.terminalIds.length <= 1;
      const fallbackExitWrite = () =>
        api.terminal
    ...
    [activeThreadId, storeCloseTerminal, terminalFeatureEnabled, terminalState.terminalIds.length],
  );
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      if (command === "terminal.toggle" && terminalFeatureEnabled) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (command === "terminal.split" && terminalFeatureEnabled) {
        event.preventDefault();
        event.stopPropagation();
        if (!terminalState.terminalOpen) {
          setTerminalOpen(true);
      ...
      if (command === "terminal.close" && terminalFeatureEnabled) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (command === "terminal.new" && terminalFeatureEnabled) {
        event.preventDefault();
        event.stopPropagation();
        if (!terminalState.terminalOpen) {
          setTerminalOpen(true);
      ...
      if (command === "diff.toggle" && diffFeatureEnabled) {
        event.preventDefault();
        event.stopPropagation();
        onToggleDiff();
        return;
  ...
  const onOpenTurnDiff = useCallback(
    ...
    [diffFeatureEnabled, navigate, threadId],
  );
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-background text-muted-foreground/40">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <p className="text-sm">
              {nexEmbedded && !threadCreationEnabled
    ...
    >
      ...
      >
        <ChatHeader
          ...
          terminalAvailable={terminalFeatureEnabled && activeProject !== undefined}
          terminalOpen={terminalFeatureEnabled && terminalState.terminalOpen}
          ...
          contextPanelEnabled={nexEmbedded}
          contextPanelOpen={contextSheetOpen}
          onToggleContextPanel={() => setContextSheetOpen((current) => !current)}
        />
      </header>
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            <div
              ...
              data-testid="chat-transcript-panel"
          ...
          {gitFeatureEnabled && pullRequestDialogState ? (
            <Suspense fallback={null}>
      ...
      {nexEmbedded ? (
        <NexContextSheet
          laneId={activeThread.id}
          laneTitle={activeThread.title}
