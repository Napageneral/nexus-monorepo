...
function rootLaneIdFromProjectId(projectId: ProjectId): ThreadId | null {
  const rawProjectId = String(projectId);
  if (!rawProjectId.startsWith("project:")) {
    return null;
  }
  return ThreadId.makeUnsafe(rawProjectId.slice("project:".length));
}
...
export default function Sidebar() {
  ...
  const sidebarProjects = useMemo<SidebarProjectSnapshot[]>(
    () =>
      orderedProjects.map((project) => {
        const rootLaneId = rootLaneIdFromProjectId(project.id);
        const projectThreadIds = threadIdsByProjectId[project.id] ?? [];
        const selectedNestedLane =
          nexEmbedded &&
          rootLaneId !== null &&
          routeThreadId !== null &&
          routeThreadId !== rootLaneId &&
          projectThreadIds.includes(routeThreadId);

        return {
          ...project,
          expanded: projectExpandedById[project.id] ?? (!nexEmbedded || selectedNestedLane),
        };
      }),
    [nexEmbedded, orderedProjects, projectExpandedById, routeThreadId, threadIdsByProjectId],
  );
  ...
  const renderedProjects = useMemo(
    () =>
      sortedProjects.map((project) => {
        ...
        const rootLaneId = rootLaneIdFromProjectId(project.id);
        const nestedProjectThreads =
          nexEmbedded && rootLaneId !== null
            ? projectThreads.filter((thread) => thread.id !== rootLaneId)
            : projectThreads;
        const isThreadListExpanded = expandedThreadListsByProject.has(project.id);
        const pinnedCollapsedThread =
          !project.expanded && activeThreadId
            ? (nestedProjectThreads.find((thread) => thread.id === activeThreadId) ?? null)
            : null;
        ...
        const orderedProjectThreadIds = nestedProjectThreads.map((thread) => thread.id);
        ...
        const isProjectLaneActive =
          nexEmbedded && rootLaneId !== null && routeThreadId === rootLaneId;
      ...
      <>
        <div className="group/project-header relative">
          <SidebarMenuButton
            ...
            size="sm"
            isActive={isProjectLaneActive}
            data-testid="chat-group-toggle"
            data-group-lane-id={project.id.startsWith("project:") ? project.id.slice("project:".length) : project.id}
            ...
            {!nexEmbedded ? (
              ...
              projectStatus ? (
                <span
                  aria-hidden="true"
                  title={projectStatus.label}
                  className={`relative inline-flex size-3 shrink-0 items-center justify-center ${projectStatus.colorClass}`}
                >
                  <span
                    className={`size-[7px] rounded-full ${projectStatus.dotClass} ${
                      projectStatus.pulse ? "animate-pulse" : ""
                    }`}
                  />
                </span>
          ...
          {threadCreationEnabled ? (
            ...
            <Tooltip>
                ...
                render={
                  <SidebarMenuAction
                    render={
                      <button
                        type="button"
                        aria-label={`${project.expanded ? "Collapse" : "Expand"} worker lanes for ${project.name}`}
                        data-testid="chat-group-expand-toggle"
                        data-group-lane-id={
                          project.id.startsWith("project:")
                            ? project.id.slice("project:".length)
                            : project.id
                        }
                      />
                    }
              ...
              />
              <TooltipPopup side="top">
                {project.expanded ? "Hide worker lanes" : "Show worker lanes"}
              </TooltipPopup>
            </Tooltip>
          ) : null}
        </div>
  ...
  const handleProjectTitleClick = useCallback(
    (event: MouseEvent<HTMLButtonElement>, projectId: ProjectId) => {
      ...
      if (nexEmbedded) {
        const rootLaneId = rootLaneIdFromProjectId(projectId);
        if (rootLaneId) {
          navigateToThread(rootLaneId);
          return;
        }
      }
      toggleProject(projectId);
    },
    [clearSelection, navigateToThread, nexEmbedded, selectedThreadIds.size, toggleProject],
  );
  ...
  const handleProjectTitleKeyDown = useCallback(
    ...
    [navigateToThread, nexEmbedded, toggleProject],
  );
  ...
  const projectCreationEnabled = isNexFeatureEnabled("projectCreation");
  const threadCreationEnabled = isNexFeatureEnabled("threadCreation");
  const projectSectionLabel = isNexEmbedded() ? "Agents" : "Projects";
        ...
        <>
          <SidebarContent className="gap-0">
            ...
            <SidebarGroup className="px-2 py-2">
              ...
              {projectCreationEnabled && shouldShowProjectPathEntry && (
