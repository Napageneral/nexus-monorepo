...
const toastManager = Toast.createToastManager<NexToastData>();
const anchoredToastManager = Toast.createToastManager<NexToastData>();
...
function shouldRenderForActiveThread(
  data: NexToastData | undefined,
  activeLaneId: string | null,
): boolean {
  const toastLaneId = data?.laneId;
  if (!toastLaneId) return true;
  return toastLaneId === activeLaneId;
}

function useActiveLaneIdFromLocation(): string | null {
  if (typeof window === "undefined") {
    return null;
  }
  return new URL(window.location.href).searchParams.get("lane")?.trim() ?? null;
}
...
function Toasts({ position = "top-right" }: { position: ToastPosition }) {
  const { toasts } = Toast.useToastManager<NexToastData>();
  const activeThreadId = useActiveLaneIdFromLocation();
