...
type PollBackfillConfig[T any] struct {
	InitialCursor time.Time
	Fetch         func(ctx context.Context, cursor time.Time, window BackfillWindow) (records []T, newCursor time.Time, done bool, err error)
}

func PollBackfill[T any](config PollBackfillConfig[T]) func(ctx context.Context, connectionID string, window BackfillWindow, emit EmitFunc) error {
	return func(ctx context.Context, connectionID string, window BackfillWindow, emit EmitFunc) error {
		cursor := config.InitialCursor
		if cursor.IsZero() {
			cursor = window.Since
		}
		...
		for {
			...
			records, newCursor, done, err := config.Fetch(ctx, cursor, window)
			if err != nil {
				return err
			}
			for _, record := range records {
