...
type AdapterOperations struct {
	...
	RecordsBackfill func(ctx context.Context, connectionID string, window BackfillWindow, emit EmitFunc) error
...
func runBackfill(adapter Adapter, args []string) error {
	...
	to := fs.String("to", "", "Backfill end date (ISO 8601 or YYYY-MM-DD)")
	_ = fs.String("format", "jsonl", "Output format (always jsonl)")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	window := BackfillWindow{Since: sinceTime}
	...
	LogInfo("backfill starting for connection %q %s", *connection, formatBackfillWindow(window))
	err = adapter.Operations.RecordsBackfill(ctx, *connection, window, emit)
	if err != nil {
...
func formatBackfillWindow(window BackfillWindow) string {
	base := fmt.Sprintf("since %s", window.Since.Format(time.RFC3339))
	if window.To == nil {
		return base
	}
	return fmt.Sprintf("%s to %s", base, window.To.Format(time.RFC3339))
}
