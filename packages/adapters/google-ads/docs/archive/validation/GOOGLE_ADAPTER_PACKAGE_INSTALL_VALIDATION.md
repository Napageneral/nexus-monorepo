# Google Adapter Package Install Validation

## Goal

Prove that the Google adapter is a real installable Nex adapter package and
...
- `dist/google-0.1.0.tar.gz` built successfully
- install returned `status = active`
- package health returned `healthy = true`
- restart rehydration logged `rehydrated active adapter "google" from durable package state`
