...
func TestAccountConfigRoundTrip(t *testing.T) {
	...
	accounts := &AccountsFile{
		Accounts: map[string]AccountConfig{
			"nexus-gitlab": {
				AccountID:           "nexus-gitlab",
				Provider:            "gitlab",
				Host:                "gitlab.com/api/v4",
				Token:               "secret-token",
				Username:            "octocat",
				CredentialRef:       "gitlab/octocat",
				Workspace:           "nexus",
	...
	if loaded.Accounts["nexus-gitlab"].CredentialRef != "gitlab/octocat" {
		t.Fatalf("CredentialRef = %q, want gitlab/octocat", loaded.Accounts["nexus-gitlab"].CredentialRef)
	}
...
func TestSetupSubmitCredentials(t *testing.T) {
	server := gitlabTestServer(t, http.StatusOK)
	defer server.Close()
...
func TestSetupSubmitBadCredentials(t *testing.T) {
	server := gitlabTestServer(t, http.StatusUnauthorized)
	defer server.Close()
...
func TestSetupSubmitRepoSelection(t *testing.T) {
	...
	if result.AccountContact.Platform != "gitlab" || result.AccountContact.SpaceID != server.URL || result.AccountContact.ContactID != "octocat" {
		t.Fatalf("AccountContact = %#v, want gitlab/%s/octocat", result.AccountContact, server.URL)
	}
	if result.SecretFields["token"] != "glpat_test" {
		t.Fatalf("secret_fields.token = %q, want glpat_test", result.SecretFields["token"])
	}
	...
	if account.Provider != "gitlab" {
		t.Fatalf("Provider = %q, want gitlab", account.Provider)
	}
...
func TestHealthConnected(t *testing.T) {
	server := gitlabTestServer(t, http.StatusOK)
	defer server.Close()
	setRuntimeContext(t, "nexus-gitlab", map[string]any{
		"workspace":             "nexus",
	...
	health, err := newGitAdapter().Health(context.Background(), "nexus-gitlab")
	if err != nil {
	...
	if health.AccountContact.Platform != "gitlab" || health.AccountContact.SpaceID != server.URL || health.AccountContact.ContactID != "octocat" {
		t.Fatalf("AccountContact = %#v, want gitlab/%s/octocat", health.AccountContact, server.URL)
	}
	if health.Details["provider"] != "gitlab" {
		t.Fatalf("provider detail = %#v, want gitlab", health.Details["provider"])
	}
...
func TestHealthDisconnected(t *testing.T) {
	server := gitlabTestServer(t, http.StatusUnauthorized)
	defer server.Close()
	setRuntimeContext(t, "nexus-gitlab", map[string]any{}, map[string]string{
		"provider": "gitlab",
		"host":     server.URL,
		"token":    "glpat_bad",
		"username": "octocat",
...
func TestAccountsList(t *testing.T) {
	...
	if accounts[0].CredentialRef != "gitlab/octocat" {
		t.Fatalf("CredentialRef = %q, want gitlab/octocat", accounts[0].CredentialRef)
	}
}

func gitlabTestServer(t *testing.T, userStatus int) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/projects", "/api/v4/projects":
			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode([]map[string]any{
				{"id": 1, "path_with_namespace": "nexus-project/nex", "name": "nex", "http_url_to_repo": "https://gitlab.com/nexus-project/nex.git", "default_branch": "main"},
				{"id": 2, "path_with_namespace": "nexus-project/spike", "name": "spike", "http_url_to_repo": "https://gitlab.com/nexus-project/spike.git", "default_branch": "main"},
			})
		default:
			http.NotFound(w, r)
		}
