	...
	methods["shopify.source.capture"] = nexadapter.Method(nexadapter.DeclaredMethod[struct{}]{
		...
		Response: map[string]any{
			...
			"properties": map[string]any{
				...
				"checkpoint": map[string]any{"type": "array", "items": map[string]any{"type": "object"}},
			},
	...
	methods["shopify.source.project-captured-page"] = nexadapter.Method(nexadapter.DeclaredMethod[struct{}]{
		...
		Params: map[string]any{
			...
			"properties": map[string]any{
				...
				"provider_response_gzip_base64": map[string]any{
					"type":      "string",
					"minLength": 1,
					"maxLength": capturedPageMaxBase64Bytes,
				},
				...
				"request_since":  map[string]any{"type": "string"},
				"window_through": map[string]any{"type": "string"},
				"record_offset": map[string]any{
					"type":    "integer",
					"minimum": 0,
					"maximum": 1_000_000,
				},
				"record_limit": map[string]any{
					"type":    "integer",
					"minimum": 1,
					"maximum": shopifySourceMaxRecords,
				},
			},
			"required": []string{
				"family",
		...
		Response: map[string]any{
			...
			"properties": map[string]any{
				...
				"record_offset":            map[string]any{"type": "integer", "minimum": 0},
				"record_limit":             map[string]any{"type": "integer", "minimum": 1, "maximum": shopifySourceMaxRecords},
				"total_records":            map[string]any{"type": "integer", "minimum": 0},
				"next_record_offset":       map[string]any{"type": "integer", "minimum": 0},
				"complete":                 map[string]any{"type": "boolean"},
				"records": map[string]any{
					"type":     "array",
					"maxItems": shopifySourceMaxRecords,
