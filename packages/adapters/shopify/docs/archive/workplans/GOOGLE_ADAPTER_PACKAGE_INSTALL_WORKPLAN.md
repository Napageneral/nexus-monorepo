# Shopify Adapter Package Install Workplan

## Customer Goal

The Shopify adapter should be packageable and restart-safe as a shared Nex
...
- shared Shopify adapter behavior
...
- emit `dist/shopify-0.1.0.tar.gz`
...
- `go test ./...` passed
- `./scripts/package-release.sh` emitted `dist/shopify-0.1.0.tar.gz`
- isolated runtime install via `/api/operator/packages/install` returned `status = active`
- package health reported `healthy = true`
- restart rehydration logged `rehydrated active adapter "shopify" from durable package state`
