...
export function parseHiddenFieldBridgePayload(input = {}, options = {}) {
  const source = input && typeof input === "object" && !Array.isArray(input) ? input : {};
  const prefix =
    typeof options.prefix === "string" && options.prefix.trim().length > 0
      ? options.prefix.trim()
  ...
  for (const key of BRIDGE_FIELD_KEYS) {
    ...
    if (prefixedKey in source) {
      extracted[key] = source[prefixedKey];
    }
  }
  return normalizeBridgeFields(extracted);
}
