...
import { createWebJourneyCore, WEB_JOURNEY_EVENT_NAMES } from "./index.mjs";
...
test("web journey core persists browser and session identity in granted mode", () => {
  const localStorage = createMemoryStorage();
  const sessionStorage = createMemoryStorage();
  const core = createWebJourneyCore({
  ...
  assert.equal(event.web_installation_id, "install-123");
  assert.equal(event.browser_id, "uuid-1");
  assert.equal(event.session_id, "uuid-1");
  assert.equal(event.event_name, WEB_JOURNEY_EVENT_NAMES.page_view);
...
test("web journey core degrades browser identity when consent is denied", () => {
  const core = createWebJourneyCore({
    web_installation_id: "install-123",
...
test("web journey core rotates session and supports handoff bridge fields", () => {
  const sessionStorage = createMemoryStorage();
  const core = createWebJourneyCore({
    web_installation_id: "install-123",
    consent_state: "granted",
...
test("web journey core updates consent state for later events", () => {
  const localStorage = createMemoryStorage();
  const core = createWebJourneyCore({
    web_installation_id: "install-123",
