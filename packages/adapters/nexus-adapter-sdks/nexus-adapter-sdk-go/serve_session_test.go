...
func TestServeSessionServe_InvokeRequest(t *testing.T) {
	...
	session := NewServeSession(input, &out)

	err := session.Serve(context.Background(), ServeHandlers{
		OnInvoke: func(_ context.Context, frame AdapterServeInvokeRequestFrame) (*AdapterServeInvokeResultFrame, error) {
			if frame.Command != "camera.snap" {
				t.Fatalf("unexpected command: %s", frame.Command)
			}
			return &AdapterServeInvokeResultFrame{
				OK:      true,
...
func TestServeEndpointRegistry_UpsertRemove(t *testing.T) {
	var out bytes.Buffer
	session := NewServeSession(strings.NewReader(""), &out)
	registry := NewServeEndpointRegistry(session)

	if err := registry.Upsert(AdapterServeEndpoint{
