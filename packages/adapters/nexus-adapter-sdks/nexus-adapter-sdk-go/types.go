...
// AdapterServeEndpoint is emitted via endpoint.upsert to declare an invokable endpoint.
type AdapterServeEndpoint struct {
...
// AdapterServeInvokeError is an optional structured error for invoke.result.
type AdapterServeInvokeError struct {
	Code    string `json:"code,omitempty"`
	Message string `json:"message,omitempty"`
}

// AdapterServeInvokeRequestFrame is sent from runtime -> adapter over adapter.serve.start stdin.
type AdapterServeInvokeRequestFrame struct {
...
// AdapterServeInvokeCancelFrame is sent from runtime -> adapter to cancel an invoke.
type AdapterServeInvokeCancelFrame struct {
	Type      string `json:"type"` // "invoke.cancel"
	RequestID string `json:"request_id"`
}

// AdapterServeEndpointUpsertFrame is sent adapter -> runtime to register/update an endpoint.
type AdapterServeEndpointUpsertFrame struct {
	Type string `json:"type"` // "endpoint.upsert"
	AdapterServeEndpoint
}

// AdapterServeEndpointRemoveFrame is sent adapter -> runtime to remove an endpoint.
type AdapterServeEndpointRemoveFrame struct {
	Type       string `json:"type"` // "endpoint.remove"
	EndpointID string `json:"endpoint_id"`
}

// AdapterServeInvokeResultFrame is sent adapter -> runtime to resolve invoke requests.
type AdapterServeInvokeResultFrame struct {
	Type      string                   `json:"type"` // "invoke.result"
	RequestID string                   `json:"request_id"`
	OK        bool                     `json:"ok"`
	Payload   any                      `json:"payload,omitempty"`
	Error     *AdapterServeInvokeError `json:"error,omitempty"`
}

// AdapterServeRecordIngestFrame is sent adapter -> runtime to ingest canonical record envelopes.
type AdapterServeRecordIngestFrame struct {
