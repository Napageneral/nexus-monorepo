...
func TestAdapterProtocolContract_FixturesValidate(t *testing.T) {
	...
	for _, f := range loadFixtureJSONL(t, "control_output_frames.jsonl") {
		requireValid(t, compileContractRef(t, "AdapterServeOutputFrame"), f)
	}
}
...
func TestAdapterProtocolContract_GoTypesRoundTrip(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("marshal runtime_context fixture: %v", err)
	}
	var ctx RuntimeContext
	if err := json.Unmarshal(raw, &ctx); err != nil {
		t.Fatalf("unmarshal runtime_context fixture: %v", err)
	}
	if strings.TrimSpace(ctx.ConnectionID) == "" {
		t.Fatalf("expected runtime context connection_id to populate RuntimeContext.ConnectionID")
	}
}
