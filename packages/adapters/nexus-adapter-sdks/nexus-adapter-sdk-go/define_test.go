...
func TestDefineAdapterBuildsInfoAndDefaults(t *testing.T) {
	...
	if info.Methods[0].Origin.PackageID != "jira" {
		t.Fatalf("origin.package_id = %q", info.Methods[0].Origin.PackageID)
	}
	if info.Methods[0].Origin.PackageVersion != "1.0.0" {
		t.Fatalf("origin.package_version = %q", info.Methods[0].Origin.PackageVersion)
	}
