...
curl --silent --show-error --fail-with-body \
  ...
  "$RUNTIME_URL/api/operator/packages/upload?operation_id=$OPERATION_ID&filename=wwex-speedship-0.1.0.tar.gz" \
  > "$UPLOAD_RECEIPT"
...
REQUEST="$(jq -cn \
  ...
  '{kind:"adapter",package_id:"wwex-speedship",version:"0.1.0",release_id:$release_id,operation_id:$operation_id,staged_artifact:{server_path:$server_path,sha256:$sha256,size_bytes:$size_bytes}}')"
...
jq -e '.ok == true and .package_id == "wwex-speedship" and .version == "0.1.0" and .status == "active"' "$INSTALL_RECEIPT" >/dev/null
