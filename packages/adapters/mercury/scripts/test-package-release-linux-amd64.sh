...
suffix="${PPID}-$$"
cleanroom_image="mercury-release-cleanroom:${suffix}"
runner_temp_root="$(cd "${TMPDIR:-/tmp}" && pwd -P)"
runner_temp="$(mktemp -d "${runner_temp_root}/mercury-release-cleanroom.XXXXXX")"
...
docker run --rm \
  ...
  --workdir /workspace/packages/adapters/mercury \
  --entrypoint bash \
  "${cleanroom_image}" \
  -c 'export PATH=/usr/local/go/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin &&
       ...
       generated_openapi_sha256="$(node ./scripts/generate-reviewed-openapi.mjs)" &&
       expected_openapi_sha256="$(node -p "require(\"./api/openapi.lock.json\").packaged_openapi_sha256")" &&
       test "${generated_openapi_sha256}" = "${expected_openapi_sha256}" &&
       go test ./... -count=1 &&
       go vet ./... &&
       PACKAGE_NEXUS_ENTRY=/opt/nex/nexus.mjs ./scripts/package-release.sh &&
       version="$(node -p "require(\"./adapter.nexus.json\").version")" &&
       archive="dist/mercury-${version}.tar.gz" &&
       normalized_root="$(mktemp -d "${TMPDIR}/mercury-release.XXXXXX")" &&
       tar -xzf "${archive}" -C "${normalized_root}" &&
       tar --sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner --format=gnu \
         -czf "${archive}.normalized" -C "${normalized_root}" . &&
       mv -f -- "${archive}.normalized" "${archive}" &&
       digest="$(sha256sum "${archive}" | cut -d" " -f1)" &&
       printf "%s  %s\n" "${digest}" "$(basename "${archive}")" > "${archive}.sha256" &&
       printf "deterministic_release_sha256=%s\n" "${digest}"'
