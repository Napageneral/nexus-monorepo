...
func parseGitMethodTarget(targetRef gitMethodTarget, methodName string) (deliveryTarget, error) {
	...
	switch methodName {
	case "github.pull_requests.create", "github.branches.create":
		if prID != "" {
	...
	case "github.pull_requests.comments.create", "github.pull_requests.merge":
		if prID == "" {
...
func executeGitMethod(ctx context.Context, req nexadapter.AdapterMethodRequest, methodName string) (*gitMethodResult, error) {
	...
	case "github.pull_requests.comments.create":
	...
	case "github.pull_requests.merge":
