...
export type ServerClass = "standard" | "compliant";
export type ServerDeploymentClass = "customer_server" | "product_control_plane";
...
export function getServerRuntimeUrl(server: ServerRecord): string | null {
  const transportHost = server.transportHost?.trim() || server.privateIp?.trim() || "";
  if (!transportHost || !server.runtimePort) return null;
...
export function getServerRuntimeWsUrl(server: ServerRecord): string | null {
  const transportHost = server.transportHost?.trim() || server.privateIp?.trim() || "";
  if (!transportHost || !server.runtimePort) return null;
  return `ws://${transportHost}:${server.runtimePort}`;
}
...
export class FrontdoorStore {
  ...
  constructor(private readonly sqlitePath: string) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS frontdoor_servers (
        ...
        server_class         TEXT NOT NULL DEFAULT 'standard',
        deployment_class     TEXT NOT NULL DEFAULT 'customer_server',
    ...
    try {
      this.db.exec("ALTER TABLE frontdoor_servers ADD COLUMN server_class TEXT NOT NULL DEFAULT 'standard'");
    } catch {
      // Already exists.
    }
    try {
      this.db.exec("ALTER TABLE frontdoor_servers ADD COLUMN deployment_class TEXT NOT NULL DEFAULT 'customer_server'");
    } catch {
      // Already exists.
    }
    try {
    ...
    try {
      this.db.exec("ALTER TABLE frontdoor_servers ADD COLUMN transport_host TEXT");
    } catch {
      // Already exists.
    }
    try {
    ...
    this.db.exec(`
      ...
      SET server_class = 'standard'
      WHERE server_class IS NULL OR TRIM(server_class) = ''
    `);
    this.db.exec(`
      UPDATE frontdoor_servers
      SET deployment_class = 'customer_server'
      WHERE deployment_class IS NULL OR TRIM(deployment_class) = ''
    `);
    this.db.exec(`
      UPDATE frontdoor_servers
      SET transport_host = private_ip
      WHERE (transport_host IS NULL OR TRIM(transport_host) = '')
  ...
  private mapServerRow(row: ServerRow): ServerRecord {
    ...
    const serverClass = row.server_class;
    const deploymentClass = row.deployment_class;
    return {
      ...
      status:
        ...
        status === "destroyed"
          ? status
          : "provisioning",
      serverClass: serverClass === "compliant" ? "compliant" : "standard",
      deploymentClass:
        deploymentClass === "product_control_plane" ? "product_control_plane" : "customer_server",
      plan: row.plan,
      provider: row.provider,
      ...
      transportHost: row.transport_host ?? row.private_ip ?? null,
  ...
  upsertServer(record: ServerRecord): ServerRecord {
    ...
    this.db
      .prepare(
        ...
        INSERT INTO frontdoor_servers (
          server_id, account_id, tenant_id, display_name, generated_name,
          status, server_class, deployment_class, plan, provider, provider_server_id, previous_provider_server_id,
          private_ip, transport_host, public_ip, previous_private_ip, previous_public_ip,
  ...
  createServer(input: {
    ...
    const now = nowMs();
    const serverClass = input.serverClass === "compliant" ? "compliant" : "standard";
    const deploymentClass =
      input.deploymentClass === "product_control_plane" ? "product_control_plane" : "customer_server";
  ...
  updateServer(serverId: string, updates: Partial<{
    ...
    if (updates.serverClass !== undefined) {
      fields.push("server_class = ?");
      values.push(updates.serverClass);
    }
    if (updates.deploymentClass !== undefined) {
      fields.push("deployment_class = ?");
      values.push(updates.deploymentClass);
    }
    if (updates.plan !== undefined) {
      fields.push("plan = ?");
      values.push(updates.plan.trim());
    }
    ...
    if (updates.transportHost !== undefined) {
      fields.push("transport_host = ?");
      values.push(updates.transportHost);
    }
  ...
  getPackageRelease(releaseId: string): FrontdoorPackageReleaseRecord | null {
    const row = this.db.prepare(`
      SELECT
        release_id,
        package_id,
        ...
        published_at_ms,
        created_at_ms
      FROM frontdoor_package_releases
      WHERE release_id = ?
      LIMIT 1
    `).get(releaseId.trim()) as
      | {
          release_id: string;
          package_id: string;
          version: string;
