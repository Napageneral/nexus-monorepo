...
function resolveServerIdFromStripeObject(payload: Record<string, unknown>): string | undefined {
  ...
  const metadata =
    metadataRaw && typeof metadataRaw === "object" && !Array.isArray(metadataRaw)
      ...
      : null;
  const serverId = typeof metadata?.server_id === "string" ? metadata.server_id.trim() : "";
  if (serverId) {
    return serverId;
  }
  return undefined;
...
export async function createCheckoutSession(params: {
  ...
  if (provider === "mock") {
    ...
    const checkoutUrl = `${params.config.baseUrl.replace(/\/+$/, "")}/billing/mock-checkout?server_id=${encodeURIComponent(
      params.serverId,
  ...
  body.set("metadata[server_id]", params.serverId);
...
export function verifyWebhookAndParseEvent(params: {
  ...
  if (provider === "mock") {
    ...
    const serverId =
      typeof parsed.server_id === "string" && parsed.server_id.trim()
        ? parsed.server_id.trim()
        : undefined;
  ...
  return {
    ...
    eventId,
    eventType,
    serverId: resolveServerIdFromStripeObject(parsed),
    payload: parsed,
  };
}
