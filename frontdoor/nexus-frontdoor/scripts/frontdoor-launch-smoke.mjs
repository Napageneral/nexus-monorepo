...
async function main() {
  ...
  const serverId = text(process.env.FRONTDOOR_SMOKE_SERVER_ID) || "tenant-dev";
  ...
  const selectServer = await postJson(
    `${origin}/api/servers/select`,
    { server_id: serverId },
    { cookie: sessionCookie },
  );
  if (!selectServer.resp.ok || !selectServer.body?.ok) {
    fail(
      `server select failed: status=${selectServer.resp.status} body=${selectServer.raw}`,
    );
  }

  const diagnostics = await getJson(
    `${origin}/api/servers/${encodeURIComponent(serverId)}/launch-diagnostics`,
    { cookie: sessionCookie },
  );
  ...
  const launchResp = await fetch(
    `${origin}${selectedApp.entry_path}?server_id=${encodeURIComponent(serverId)}`,
    {
      headers: { cookie: sessionCookie },
      redirect: "manual",
    },
