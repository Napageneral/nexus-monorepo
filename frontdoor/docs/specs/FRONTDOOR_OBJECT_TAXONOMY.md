...
7. `server_class` is the customer-facing hosting-policy term that drives
   compliant-vs-standard behavior.
...
| `server_class` | Customer-facing hosting policy for a server: `standard` or `compliant` |
| `standard server` | Lower-cost hosted server class that defaults to Hetzner |
| `compliant server` | Regulated hosted server class that runs on AWS |
...
4. `server_class` defines provider policy and install guardrails without
   changing the durable `server_id`
