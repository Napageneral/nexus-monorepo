...
- **Server classes**: Users choose `standard` or `compliant` as the hosting policy. `standard` is the lower-cost default; `compliant` is the AWS-only regulated class for HIPAA-sensitive workloads.
...
2. Select server class:
   - `standard`
   - `compliant`
3. Select server tier/size within the allowed class options
4. Billing: server creation is allowed by active free tier or available credits; paid usage draws from the account credit balance
5. New server provisions → appears in server list with its class badge
6. User can then install apps allowed on that class
...
2. Server detail view shows:
   ...
   - Server class badge (`standard` or `compliant`)
...
4. If the app requires a compliant server and the selected server is standard:
   a. install is blocked before package staging
   b. frontdoor prompts the user to choose or create a compliant server
5. Installation mechanics (see `FRONTDOOR_PACKAGE_REGISTRY_AND_LIFECYCLE.md`):
...
6. Status transitions: not_installed → installing → installed (or failed)
7. Visual progress during installation
8. Once installed, app card on server detail shows "Launch" button
