...
6. Apps may depend on separately installable adapter and app packages.
...
3. No silent fallback from requested app to operator console.
...
3. Frontdoor backend itself runs in AWS as the canonical hosted control plane.
4. Server class, not user-facing cloud choice, determines provider policy:
...
8. `FRONTDOOR_AWS_HOSTING_AND_SERVER_CLASS_MODEL.md` — AWS-hosted frontdoor baseline, `standard` vs `compliant`, and the Hetzner/AWS provider split.
...
1. `CLOUD_PROVISIONING_ARCHITECTURE_2026-03-04.md` — cloud provider abstraction, server-class provider policy, snapshot strategy, and frontdoor infrastructure behavior under the durable server lifecycle.
