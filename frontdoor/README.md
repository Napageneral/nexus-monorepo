...
Canonical gateway for the Nexus hosted platform. Frontdoor is one hosted
control plane with one customer shell.

See `docs/specs/FRONTDOOR_ARCHITECTURE.md` for the full architecture spec.

## Current Hosted Model

- Frontdoor backend runs in AWS.
- Frontdoor is the public edge for `frontdoor.nexushub.sh` and tenant
  subdomains.
- Authentication is OIDC-first in production.
- Customers choose server class, not cloud vendor:
  - `standard`
  - `compliant`
- `standard` servers default to Hetzner and are reached from AWS frontdoor over
  Tailscale.
- `compliant` servers are AWS-only.
- The active hosted shell is rendered by Frontdoor itself. There is no separate
  launcher frontend.
...
- **Servers**: Provisioning and lifecycle of isolated nex runtime instances
  across `standard` and `compliant` classes
...
- OIDC-first login
- one hosted shell
- `standard` / `compliant` server classes
...
- `/Users/tyler/nexus/home/projects/nexus/frontdoor/.demo-stack/stack.env`
...
- Tenant runtimes are provisioned with hosted trusted-token auth and operator console assets. The provisioning script will build operator console assets if missing.

## Config

Config file:

- `/Users/tyler/nexus/home/projects/nexus/frontdoor/config/frontdoor.config.json`
...
- `FRONTDOOR_TENANT_OPERATOR_CONSOLE_ROOT=/abs/path/to/nex/dist/operator-console`
...
- `FRONTDOOR_SMOKE_USERNAME` / `FRONTDOOR_SMOKE_PASSWORD` for local password-enabled environments only
...
Password auth may still be useful for local/dev-only environments where it is
explicitly enabled. It is not the primary hosted production auth surface.
