...
function buildConfig(params: { origin: string; frontdoorStorePath: string }): FrontdoorConfig {
  ...
  return {
    ...
    appStoragePath: path.join(os.tmpdir(), `frontdoor-local-sandbox-package-${randomUUID()}`),
  };
}
...
function seedApiToken(config: FrontdoorConfig): string {
  ...
  try {
    ...
    store.addCredits({
      ...
      amountCents: 500,
      type: "deposit",
      description: "local sandbox package lifecycle pilot credits",
    });
    store.createApiToken({
      ...
      displayName: "Local Sandbox Package Pilot",
    });
  } finally {
    store.close();
  }
...
function readPackageMetadata(packageRoot: string): { packageId: string; version: string; tarballPath: string } {
  ...
  if (!packageId || !version) {
    throw new Error(`invalid_app_manifest:${manifestPath}`);
  }
  return {
    packageId,
    version,
...
async function publishLocalAppRelease(params: {
  ...
  try {
    return await publishAppRelease({
      store,
...
async function main(): Promise<void> {
  ...
  const defaultPackageRoot = path.join(repoRoot, "packages", "apps", "spike", "app");
  ...
  const proofBundleDir =
    ...
    fs.mkdtempSync(path.join(os.tmpdir(), "frontdoor-local-sandbox-package-proof-"));
  ...
  const publishResult = await publishLocalAppRelease({
    config,
    packageRoot,
  ...
  try {
    ...
    const wrapper = await execFileAsync(
      ...
      [
        ...
        "node",
        "./scripts/frontdoor-fresh-server-package-lifecycle-smoke.mjs",
      ],
      {
        cwd: frontdoorDir,
        env: {
          ...
          FRONTDOOR_SMOKE_APP_ID: packageRelease.packageId,
          FRONTDOOR_SMOKE_PURCHASE: "true",
          FRONTDOOR_SMOKE_UNINSTALL: "true",
    ...
    const proofPath = path.join(proofBundleDir, "proof/fresh-server-package-lifecycle-smoke.json");
