...
export function resolveGatewayLogPaths(
  ...
  const home = resolveHomeDir(env);
  const stateDir = env.NEXUS_STATE_DIR?.trim();

  // Prefer the configured state dir when provided; otherwise default to ~/nexus/state/logs
  const logDir = stateDir
    ? path.join(path.resolve(stateDir), "logs")
    : path.join(home, "nexus", "state", "logs");
  return {
    logDir,
