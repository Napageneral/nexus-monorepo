...
func TestControlStartSystemWhich(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-which","endpoint_id":"headless-default","command":"system.which","payload":{"bins":["sh","definitely-not-a-real-bin"]}}` + "\n")
	var out strings.Builder
	session := nexadapter.NewControlSession(input, &out)

	if err := controlStart(context.Background(), "default", session); err != nil {
		t.Fatalf("controlStart() error: %v", err)
	}
	...
	if lines[1]["type"] != "invoke.result" || lines[1]["ok"] != true {
		t.Fatalf("expected successful invoke.result, got %#v", lines[1])
	}
}

func TestControlStartSystemRun(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-run","endpoint_id":"headless-default","command":"system.run","payload":{"command":["sh","-lc","printf hello"]}}` + "\n")
	var out strings.Builder
	session := nexadapter.NewControlSession(input, &out)
	...
	if !ok {
		t.Fatalf("expected payload object, got %#v", lines[1]["payload"])
	}
	result, ok := payload["result"].(map[string]any)
	if !ok {
		t.Fatalf("expected payload.result object, got %#v", payload["result"])
	}
...
func TestControlStartRejectsUnknownCommand(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-unknown","endpoint_id":"headless-default","command":"camera.snap","payload":{}}` + "\n")
	var out strings.Builder
	session := nexadapter.NewControlSession(input, &out)
...
func TestHeadlessCommandsHonorBrowserToggle(t *testing.T) {
	t.Setenv("NEXUS_DEVICE_HEADLESS_BROWSER_PROXY_ENABLED", "false")
	commands := headlessCommands()
	for _, command := range commands {
		if command == "browser.proxy" {
			t.Fatalf("expected browser.proxy to be disabled")
		}
	}
}
