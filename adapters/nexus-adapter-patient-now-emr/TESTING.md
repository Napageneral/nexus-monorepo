# PatientNow EMR Adapter Testing

## Local package/install bar

1. `go test ./...`
2. `go build -o ./bin/patient-now-emr-adapter ./cmd/patient-now-emr-adapter`
3. `./scripts/package-release.sh`
4. install the emitted tarball through `POST /api/operator/packages/install`
5. verify package health through `GET /api/operator/packages/adapter/nexus-adapter-patient-now-emr/health`
