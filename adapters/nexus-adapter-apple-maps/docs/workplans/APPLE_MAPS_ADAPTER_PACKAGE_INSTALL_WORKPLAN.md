# Apple Maps Adapter Package Install Workplan

## Customer Goal

The Apple Maps adapter should be packageable and restart-safe as a shared Nex
adapter package.
...
- emit `dist/nexus-adapter-apple-maps-0.1.0.tar.gz`

### Phase 3: Package Docs

- add `docs/specs/APPLE_MAPS_ADAPTER_PACKAGE_DISTRIBUTION_AND_INSTALL.md`
- add `docs/workplans/APPLE_MAPS_ADAPTER_PACKAGE_INSTALL_WORKPLAN.md`
- add `docs/validation/APPLE_MAPS_ADAPTER_PACKAGE_INSTALL_VALIDATION.md`
...
- manual Apple Maps fixture validation
...
- `go test ./...` passed
- `./scripts/package-release.sh` emitted `dist/nexus-adapter-apple-maps-0.1.0.tar.gz`
- isolated runtime install via `/api/operator/packages/install` returned `status = active`
- package health reported `healthy = true`
- restart rehydration logged `rehydrated active adapter "nexus-adapter-apple-maps" from durable package state`
