...
function normalizeCredential(cred) {
	...
	if (v2.success) {
		...
		const fields = rec.fields && typeof rec.fields === "object" && !Array.isArray(rec.fields) ? rec.fields : void 0;
		const authID = typeof rec.auth_id === "string" ? rec.auth_id : void 0;
	...
	try {
		switch (command) {
			...
			case "event.backfill": {
				...
				if (!handler) throw new Error("event.backfill not supported by this adapter");
				const account = requireFlag(filteredArgs, "--account");
			...
			case "delivery.send": {
				...
				if (!handler) throw new Error("delivery.send not supported by this adapter");
				const account = requireFlag(filteredArgs, "--account");
				...
				if (!text && !media) throw new Error("delivery.send requires --text or --media");
				if (text && media) throw new Error("delivery.send must not specify both --text and --media");
				if (caption && !media) throw new Error("delivery.send --caption requires --media");
				const req = {
					account,
					to,
					...text ? { text } : {},
			...
			case "delivery.stream": {
				...
				if (!handler) throw new Error("delivery.stream not supported by this adapter");
