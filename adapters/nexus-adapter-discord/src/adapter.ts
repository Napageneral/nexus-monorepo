...
type BackfillOptions = {
  ...
  autodiscover_guild_channels: boolean;
};
...
function normalizeContainer(message: Message): NormalizedContainer {
...
function isBackfillableChannelType(type: number): boolean {
  return (
    type === ChannelType.DM ||
...
function normalizeContainerFromChannelRecord(
  channel: DiscordChannelRecord,
  parent?: DiscordChannelRecord,
): NormalizedContainer {
...
function toDiscordChannelRecord(value: unknown): DiscordChannelRecord | null {
...
function parseRawMessageTimestamp(raw: UnknownRecord): number | null {
...
function parseRawReplyToID(raw: UnknownRecord): string | undefined {
...
function parseRawMentionsBot(raw: UnknownRecord, selfID: string): boolean {
  if (!selfID) {
    return false;
  }
...
function parseRawMessage(raw: unknown, selfID: string): {
  id: string;
  timestamp_ms: number;
  content: string;
  ...
  const senderName =
    asString(member?.nick) ?? asString(author?.global_name) ?? asString(author?.username);
...
function restClient(token: string): REST {
...
function renderStreamChunks(text: string): string[] {
  if (!text.length) {
...
async function renderDiscordStreamState(params: {
  ...
  for (let idx = 0; idx < chunks.length; idx++) {
    ...
    const createdID = await createDiscordMessage(
      params.rest,
      params.state.channel_id,
      chunk,
      idx === 0 ? params.state.reply_to_id : undefined,
    );
...
function collectConfiguredBackfillChannelIDs(config: UnknownRecord | undefined): string[] {
...
function resolveBackfillOptions(ctx: AdapterContext): BackfillOptions {
  const config = asRecord(ctx.runtime?.config);
  const backfillConfig = asRecord(config?.backfill);
  const channelIDs = collectConfiguredBackfillChannelIDs(config);

  const maxMessages =
    asPositiveInt(backfillConfig?.max_messages_per_channel) ??
    asPositiveInt(config?.backfill_max_messages_per_channel) ??
    DEFAULT_BACKFILL_MAX_MESSAGES_PER_CHANNEL;
  const includeBots =
    asBoolean(backfillConfig?.include_bots) ?? asBoolean(config?.backfill_include_bots) ?? false;
  const autodiscover =
    asBoolean(backfillConfig?.autodiscover_guild_channels) ??
...
async function fetchSelfUserID(rest: REST): Promise<string> {
  const me = (await rest.get(Routes.user())) as { id?: string };
  return me.id?.trim() ?? "";
}

async function discoverGuildChannelIDs(rest: REST, ctx: AdapterContext): Promise<string[]> {
  ...
  for (const guild of guilds) {
    ...
    try {
      const channels = (await rest.get(Routes.guildChannels(guildID))) as unknown;
      ...
      ctx.log.info(
        "discord backfill: guild discovery failed guild=%s err=%s",
        guildID,
...
async function getChannelRecord(
  rest: REST,
  channelID: string,
  cache: Map<string, DiscordChannelRecord>,
): Promise<DiscordChannelRecord | null> {
  const existing = cache.get(channelID);
  if (existing) {
    return existing;
  }
  const raw = (await rest.get(Routes.channel(channelID))) as unknown;
...
async function getGuildName(
  ...
  const raw = (await rest.get(Routes.guild(guildID))) as unknown;
  const record = asRecord(raw);
  const name = asString(record?.name);
  if (name) {
...
async function backfillChannelMessages(params: {
  ...
  while (!params.ctx.signal.aborted && emitted < params.max_messages) {
    ...
    const newestTimestamp = parsedBatch.reduce(
      (acc, entry) => Math.max(acc, entry.timestamp_ms),
...
let activeStream: DiscordStreamState | null = null;
let activeStreamRest: REST | null = null;

const discordStreamHandlers: StreamHandlers = {
  ...
  onEnd: async (_ctx, event) => {
    ...
    if (event.runId !== activeStream.run_id) {
      throw new Error(
        `discord stream run mismatch: expected ${activeStream.run_id}, got ${event.runId}`,
      );
    }
    ...
    const finalMessageID =
      activeStream.message_ids[activeStream.message_ids.length - 1] ?? activeStream.message_ids[0];
    if (finalMessageID) {
      emitStreamStatus({
        type: "message_sent",
...
export const discordAdapter = defineAdapter<{ token: string; rest: REST }>({
  platform: "discord",
