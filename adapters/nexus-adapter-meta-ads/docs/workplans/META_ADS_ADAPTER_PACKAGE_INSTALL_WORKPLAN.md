# Meta Ads Adapter Package Install Workplan

## Customer Goal

The Meta Ads adapter should be packageable and restart-safe as a shared Nex
adapter package.
...
- emit `dist/nexus-adapter-meta-ads-0.1.0.tar.gz`

### Phase 3: Package Docs

- add `docs/specs/META_ADS_ADAPTER_PACKAGE_DISTRIBUTION_AND_INSTALL.md`
- add `docs/workplans/META_ADS_ADAPTER_PACKAGE_INSTALL_WORKPLAN.md`
- add `docs/validation/META_ADS_ADAPTER_PACKAGE_INSTALL_VALIDATION.md`
...
- `go test ./...` passed
- `./scripts/package-release.sh` emitted `dist/nexus-adapter-meta-ads-0.1.0.tar.gz`
- isolated runtime install via `/api/operator/packages/install` returned `status = active`
- package health reported `healthy = true`
- restart rehydration logged `rehydrated active adapter "nexus-adapter-meta-ads" from durable package state`
