# Twilio Adapter Package Install Validation

## Goal

Prove that the Twilio adapter is a real installable Nex adapter package and
...
- `dist/nexus-adapter-twilio-0.1.0.tar.gz` built successfully
- initial install failed because the api-key auth method omitted `service`
- after adding `service = "twilio"` to `adapter.info`, install returned `status = active`
