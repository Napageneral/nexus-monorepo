...
| --- | --- | --- | --- |
| `nexus-adapter-apple-maps` | shared Go SDK | Green | None beyond ongoing method declaration evolution |
| `nexus-adapter-callrail` | shared Go SDK | Green | None beyond ongoing method declaration evolution |
| `nexus-adapter-google` | shared Go SDK | Green | None beyond ongoing method declaration evolution |
| `nexus-adapter-meta-ads` | shared Go SDK | Green | None beyond ongoing method declaration evolution |
| `nexus-adapter-twilio` | shared Go SDK | Green | None beyond ongoing method declaration evolution |
...
| --- | --- | --- |
| `nexus-adapter-device-headless` | `AdapterControlStart`, `req.Account`, `AdapterHealth.Account` | rename to current SDK handler names and `ConnectionID` fields |
| `nexus-adapter-device-android` | same pattern as headless | same migration as headless |
| `nexus-adapter-device-macos` | same pattern as headless | same migration as headless |
| `nexus-adapter-device-ios` | same pattern as headless | same migration as headless |
| `nexus-adapter-jira` | mixed old surface: `AdapterMonitorStart`, `DeliverySend`, setup/account fields | remove CLI shim, move to current monitor/backfill/send handlers and current result/request fields |
| `nexus-adapter-patient-now-emr` | `EventBackfill`, `AdapterMonitorStart`, `NexusEvent`, old health fields | full event-surface cutover to canonical `record.ingest` |
| `nexus-adapter-gog` | `DeliverySend`, `EventBackfill`, `NexusEvent`, `NEXUS_GOG_STATE_DIR` | same as patient-now plus state-dir cutover |
| `nexus-adapter-zenoti-emr` | `EventBackfill`, `AdapterMonitorStart`, `NexusEvent` | same as patient-now |
...
| --- | --- | --- |
| `nexus-adapter-confluence` | `./nexus-adapter-sdk-go` | package behavior partly aligned, but still pinned to a private SDK fork and old field names inside that fork |
| `nexus-adapter-git` | `./nexus-adapter-sdk-go` | package code and tests still center `NexusEvent`, `DeliverySend`, `EventBackfill` |
| `slack` | `./third_party/adapter-sdk-go` | package and vendored SDK still use old Go SDK naming and request fields |
| `nexus-adapter-qase` | `./sdk/nexus-adapter-sdk-go` | package code is closer to the current surface, but still forks SDK ownership and has not yet adopted the shared workspace path |
...
| Adapter | Primary evidence | Required migration |
| --- | --- | --- |
| `nexus-adapter-discord` | imports `newEvent` and `type NexusEvent` | move to `newRecord`, `AdapterInboundRecord`, current helper names |
| `nexus-adapter-telegram` | imports `newEvent` and `type NexusEvent` | same migration as Discord |
| `nexus-adapter-whatsapp` | imports `newEvent` and `type NexusEvent` | same migration as Discord |

### Category E: Package with local SDK fork but low immediate migration pressure

| Adapter | Status | Note |
| --- | --- | --- |
| `nexus-adapter-qase` | package code appears near-current | still should converge on the shared Go SDK to stop another private contract fork from persisting |
...
1. `nexus-adapter-device-headless`
2. `nexus-adapter-device-android`
3. `nexus-adapter-device-macos`
4. `nexus-adapter-device-ios`
5. `nexus-adapter-jira`
6. `nexus-adapter-patient-now-emr`
7. `nexus-adapter-gog`
8. `nexus-adapter-zenoti-emr`
