...
- real dual-runtime proof with distinct server/runtime identities
...
## WPR5. Dual-Runtime Hosted Identity Proof ✅

Run the rehearsal against two genuinely separate runtime processes.

Exit criteria:

- control-plane server and clinic server use different runtime URLs
- frontdoor stores distinct runtime auth tokens for both servers
- the clinic runtime and control-plane runtime do not share one process

## WPR6. Control-Plane Call Validation ✅
...
This workplan is complete when WPR1-WPR6 pass with recorded evidence.
