...
export default function IntegrationsPage() {
  ...
  const [uploadTargetAdapterId, setUploadTargetAdapterId] = useState<string | null>(null);
  ...
  const connectAdapter = useCallback(
    async (adapter: IntegrationAdapter) => {
      ...
      try {
        const primaryMethod = adapter.authMethods[0];
        if (!primaryMethod) {
          setError(`No supported auth method found for ${adapter.name}.`);
          return;
        }

        if (primaryMethod.type === "oauth2") {
        ...
        if (primaryMethod.type === "api-key") {
          const fields =
            primaryMethod.fields && primaryMethod.fields.length > 0
              ? primaryMethod.fields
          ...
          setMessage(`${adapter.name} connected via API key.`);
          await loadIntegrations();
          return;
        }

        if (primaryMethod.type === "file-upload") {
          const input = uploadInputRef.current;
          if (!input) {
            setError("Upload input is unavailable in this browser.");
            return;
          }
          setUploadTargetAdapterId(adapter.id);
          input.value = "";
          input.click();
          return;
        }
  ...
  const onUploadFileSelected = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      const adapterId = uploadTargetAdapterId;
      if (!adapterId) {
        return;
      }
      const file = event.currentTarget.files?.[0] ?? null;
      if (!file) {
        setUploadTargetAdapterId(null);
        return;
      }

      setBusyAdapterId(adapterId);
      setMessage(null);
      setError(null);
      try {
        ...
        setMessage(
          `${adapter?.name ?? adapterId} upload validated (${result.preview?.rowCount ?? 0} rows detected).`,
        );
        await loadIntegrations();
      } catch (err) {
        setError(err instanceof Error ? err.message : String(err));
    ...
    [adapters, loadIntegrations, uploadTargetAdapterId],
  );
          ...
          return (
            <div key={intg.id} className="bg-gb-card border border-gb-border rounded-lg p-6">
              ...
              {intg.connected ? (
                ...
                <button
                  onClick={() => void connectAdapter(intg)}
                  disabled={isBusy}
                  className="w-full bg-gb-bg hover:bg-gb-border/50 border border-gb-border rounded-lg py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                >
                  {intg.authMethods.some((method) => method.type === "oauth2") ? <LinkIcon /> : <GearIcon />}
                  Connect
                </button>
              )}
            </div>
