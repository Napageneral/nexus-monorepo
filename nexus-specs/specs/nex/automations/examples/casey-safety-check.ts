...
export const hook: Hook = {
  ...
  handler: async (ctx: HookContext): Promise<HookResult> => {
    ...
    const messages = db.prepare(`
      SELECT e.content, e.timestamp, el.body as emotion
      FROM events e
      JOIN event_participants ep ON ep.event_id = e.id
      JOIN person_contact_links pcl ON pcl.contact_id = ep.contact_id
      JOIN persons p ON p.id = pcl.person_id
      LEFT JOIN set_members sm ON sm.event_id = e.id
      LEFT JOIN elements el ON el.set_id = sm.set_id AND el.kind = 'emotion'
