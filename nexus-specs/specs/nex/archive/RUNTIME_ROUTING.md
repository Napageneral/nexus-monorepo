...
**Archived:** 2026-02-27 — Routing model (Routing object, RoutingParticipant, Entity resolution, session_key, persona_path) is now canonical in `NEXUS_REQUEST_TARGET.md`.
...
1. **Contacts** — the delivery-driven directory that maps `(platform, space_id, contact_id)` to entities
...
A contact is a delivery endpoint — a `(platform, space_id, contact_id)` tuple representing one way to reach or be reached by someone. Every inbound message creates or updates a contact automatically at pipeline time. No LLM involved.
...
CREATE TABLE contacts (
    ...
    contact_id      TEXT NOT NULL,              -- platform-specific contact ID
...
When a new `(platform, space_id, contact_id)` tuple is seen for the first time, the pipeline:
...
function ensureContact(
  ...
  const existing = identityDb
    .prepare('SELECT entity_id FROM contacts WHERE platform = ? AND space_id = ? AND contact_id = ?')
  ...
  identityDb.prepare(`
    INSERT INTO contacts (platform, space_id, contact_id, entity_id, first_seen, last_seen, message_count, contact_name)
    VALUES (?, ?, ?, ?, ?, ?, 1, ?)
...
Facts extracted from all sessions are linked to entities via `element_entities` in memory.db (referencing entity IDs in identity.db by convention). Since the entities are now merged, all facts resolve to the same canonical entity. The memory-injection meeseeks naturally surfaces relevant facts from all sessions when building context for the primary session.
...
SELECT platform, space_id, contact_id, contact_name, avatar_url, last_seen
FROM contacts
...
The memory system V2 unified storage model (elements, sets, jobs, element_entities, etc. in memory.db) is unchanged by this spec. The entity store tables (entities, merge_candidates, entity_cooccurrences, entity_tags) are relocated to `identity.db` per DATABASE_ARCHITECTURE.md, enabling same-DB JOINs with contacts. This spec adds:
...
+ contacts table (new: platform, space_id, contact_id, entity_id NOT NULL, contact_name, ...)
