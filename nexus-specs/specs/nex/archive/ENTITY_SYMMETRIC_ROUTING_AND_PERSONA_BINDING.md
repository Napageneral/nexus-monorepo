...
**Archived:** 2026-02-27 — Entity model, symmetric sender/receiver routing, persona_path (replacing persona_bindings), and session_key are all canonical in `NEXUS_REQUEST_TARGET.md`.
...
Receiver identity resolution uses the existing `contacts` table -- the same path used for sender resolution. A contacts row with `(platform, space_id='', contact_id=<account_id>)` maps the adapter account to its receiver entity. No separate `account_receiver_bindings` table is needed; identity resolution is fully symmetric between sender and receiver.
...
5. `workplans/ENTITY_SYMMETRIC_ROUTING_AND_PERSONA_BINDING_WORKPLAN.md`
