...
NEX (Nexus Event Exchange) is the evolution of the existing Cortex server. This document maps every Cortex package to its role in NEX and identifies what needs to be built.

**Key insight:** Cortex becomes NEX. The Go infrastructure, job bus, and ledgers are the foundation. We add pipeline orchestration and plugins on top.
...
| Cortex Package | NEX Role | Changes Needed |
|------------------|----------|----------------|
...
| `internal/compute/embeddings_batcher.go` | **Batch embeddings** | Used by Cortex for background processing. |
...
| `internal/me/me.go` | **Self-knowledge** | Part of Cortex. |
...
cortex/
├── cmd/
│   └── cortex/main.go           # Entry point (becomes NEX server)
│
├── internal/
