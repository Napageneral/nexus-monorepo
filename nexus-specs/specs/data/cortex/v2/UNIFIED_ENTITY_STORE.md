...
**That's it.** No `channel`, `identifier`, `mapping_type`, `confidence`, `alias_type`, `display_name`, `relationship`, `notes`, `message_count` columns. Those are either captured by the `type` field, stored as facts in the facts table, or derivable from events.
...
|-------|------|-------------|
| `channel` | string | Platform: 'slack', 'discord', 'imessage', 'gmail' |
...
| `capabilities` | object | Channel capabilities (markdown, message length, etc.) |
| `available_channels` | array | Other channels available for response |
...
| Cortex `person_facts` table | Facts about people live in the facts table |
| Cortex `merge_candidates` table | Kept as-is |
| Hindsight `entities` table | Merged into unified entities table |
| Hindsight `entity_cooccurrences` table | Kept as-is |
