...
5. **Virtual tables to CREATE:**
   - `facts_fts` (FTS5 over facts.text)
   - `vec_embeddings` (sqlite-vec for vector search)
...
**Test:** Open the database, verify all tables exist, verify old tables are gone. Run a few inserts into facts/entities to confirm constraints work.

**Files touched:**
- `internal/db/schema.sql` — rewrite the memory system tables
- `internal/db/db.go` — update migration logic if needed
...
1. **Create new package** `internal/recall/` (or refactor `internal/search/`).
...
5. **Implement filters:**
   ...
   - `channel`: WHERE clause on `metadata->>'source_channel'`
   - `max_results`: LIMIT
...
7. **Expose as a Go function** that can be called from:
   - Tool implementations (for meeseeks)
   - The injection hook (direct call)
   - CLI for testing

**Reference:** Port Hindsight's `memory_engine.py` recall implementation. Translate pgvector → sqlite-vec, asyncpg → database/sql, Python async → Go goroutines.
...
**Files touched:**
- `internal/recall/recall.go` — main recall function
- `internal/recall/strategies.go` — individual retrieval strategies
- `internal/recall/fusion.go` — RRF + MMR
- `internal/recall/types.go` — result types
...
1. **Delete old memory pipeline code:**
...
3. **Backfill pipeline:**
   - Create a CLI command or script: `cortex backfill --channel imessage --from 2024-01-01`
   - Groups events into episodes (uses existing episode definitions, target 4-8K tokens per episode)
   - Processes episodes sequentially per channel through the Memory-Writer meeseeks
...
1. **Schema:** `sqlite3 cortex.db ".tables"` — verify table inventory
