...
Temporal and channel filtering are applied as WHERE clauses on results, not separate retrieval strategies.
...
3. **Sequential within channel, parallel across channels** -- Process a channel's episodes in chronological order so the writer builds entity context over time (earlier messages teach it who "Mom" is). Different channels (iMessage, Discord, email) process in parallel since they have independent entity contexts.
...
Cortex uses SQLite. For vector similarity search, use the `sqlite-vec` extension. Virtual table:
...
Cortex currently uses Gemini embeddings but these are expensive per-call. For V2, use the same local model as Hindsight:
...
Check `internal/bus/bus.go` and `internal/live/` for existing hook infrastructure.
