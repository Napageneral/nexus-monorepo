...
- `delivery.channel`: platform (`discord`, `telegram`, `imessage`, …)
- `delivery.peer_kind`: `dm | group | channel`
- `delivery.peer_id`: conversation container ID
...
- If NEX happens to have `{channel}:{id}`-style values, adapters **may** strip the prefix, but the canonical value is the platform ID.
...
```jsonl
{"type":"stream_start","runId":"run_abc","sessionLabel":"main","target":{"channel":"discord","account_id":"echo-bot","to":"channel:123","thread_id":"123456789012345678","reply_to_id":"987654321098765432"}}
```
...
- `peer_id`: the parent channel ID (or DM channel ID for DMs)
