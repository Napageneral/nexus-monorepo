...
- Full fidelity archival in AIX/Cortex
...
## AIX/Cortex Integration

### How Nexus Sessions Flow to Cortex
...
~/nexus/state/sessions/{id}.jsonl
      ...
      AIX (full fidelity capture)
       │
      ├─► aix-events adapter → Cortex Events Ledger
      │   (trimmed turns: user message + final response)
      │
      └─► aix-agents adapter → Cortex Agents Ledger
           (full sessions, messages, turns, tool_calls)
```

### Field Mapping: Nexus → AIX → Cortex

| Nexus Session Field | AIX sessions | Cortex agent_sessions |
|---------------------|--------------|-------------------------|
| sessionId | id | id |
| model | model | model |
...
| Nexus Message Field | AIX messages | Cortex agent_messages |
|---------------------|--------------|-------------------------|
| uuid | id | id |
...
| Nexus ToolCall | AIX tool_calls | Cortex agent_tool_calls |
|----------------|----------------|---------------------------|
| id | id | id |
...
- `docs/AIX_CORTEX_PIPELINE.md` - How AIX feeds into Cortex
...
│                                                                 │
│  Cursor  ──┐                                                    │
│  Codex   ──┼──► AIX (capture) ──► Cortex (memory)            │
│  Nexus   ──┤         │                   │                      │
...
1. Unified search across ALL agent sources in Cortex
