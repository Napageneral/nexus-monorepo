...
| `contacts` | identity.db | Platform identifiers, message counts, first/last seen |
...
| `facts` + `fact_entities` | memory.db | Knowledge graph, relationship context |
| `mental_models` | memory.db | Synthesized relationship assessments |
| `entity_cooccurrences` | identity.db | Co-mention frequency |
...
SELECT name, description, last_refreshed, is_stale
FROM mental_models
...
interface EntityProfile {
  ...
  // Relationship context (from memory)
  context: {
    recent_facts: Array<{ text: string; as_of: number }>;
    mental_model: { description: string; last_refreshed: number; is_stale: boolean } | null;
