...
**Approach:** The meeseeks dispatch (Phase 5) creates a `jobs` row before starting the meeseeks. The `jobs.id` is passed into the tool context so every tool can reference it.

```typescript
// In tool factory function:
function createMemoryWriterTools(ctx: {
    memoryDb: DatabaseSync;
    currentJobId: string;  // NEW — passed from meeseeks dispatch
    // ... other context
}) {
    // Each tool uses ctx.currentJobId for source_job_id and job_outputs
}
```
