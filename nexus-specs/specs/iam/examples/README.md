...
│                    ACL EVALUATION                                │
│                                                                  │
│  1. Resolve sender (query entities table)                       │
│  2. Match policies (sender + conditions)                        │
│  3. Check for denies (any deny → reject)                        │
│  4. Merge permissions from allows                               │
│  5. Assign session from highest priority                        │
│                                                                  │
│  Output: { sender, permissions, session } or DENY               │
└─────────────────────────────────────────────────────────────────┘
     │
     │ (if allowed)
     ▼
...
│    ctx.sender      — Who sent this (from ACL)                   │
...
triggers: {
  sender: { name: 'Mom' },
  event: { platforms: ['imessage', 'sms'], direction: 'received' }
}
```

Hook system checks:
- `ctx.sender.name === 'Mom'` → ✓
- `ctx.event.platform in ['imessage', 'sms']` → ✓
- `ctx.event.direction === 'received'` → ✓

**Result:** Triggers match! Handler will be invoked.

(Other hooks like `casey-safety-check` have `sender: { type: 'system' }` triggers, so they DON'T match and their handlers aren't invoked.)
...
ctx = {
  ...
  sender: { name: "Mom", tags: ["relationship:family"] },  // From ACL
...
if (is2FA) {
  return {
    ...
    context: {
      ...
      prompt: `${sender.name} needs the Amazon 2FA code...`
    }
  };
}
```
...
2. **Use tags** — Match entity tags for relationship-based access
...
5. **Test with CLI** — `nexus acl test --sender mom --platform imessage`
