...
func TestControlStartUpsertsEndpointAndHandlesInvoke(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-1","endpoint_id":"android-default","command":"camera.snap","payload":{"quality":"high"}}` + "\n")
...
func TestControlStartRejectsUnknownCommand(t *testing.T) {
	input := strings.NewReader(`{"type":"invoke.request","request_id":"req-2","endpoint_id":"android-default","command":"system.run","payload":{}}` + "\n")
	var out strings.Builder
	session := nexadapter.NewControlSession(input, &out)
