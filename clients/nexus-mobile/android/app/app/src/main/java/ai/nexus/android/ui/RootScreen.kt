...
import ai.nexus.android.CameraHudKind
...
fun RootScreen(viewModel: MainViewModel) {
  ...
  val activity =
    remember(cameraHud, screenRecordActive, isForeground, statusText) {
  ...
  val runtimeState =
    remember(serverName, statusText) {
      when {
        serverName != null -> RuntimeState.Connected
        statusText.contains("connecting", ignoreCase = true) ||
          statusText.contains("reconnecting", ignoreCase = true) -> RuntimeState.Connecting
        statusText.contains("error", ignoreCase = true) -> RuntimeState.Error
  ...
  AndroidView(
    ...
    factory = {
      WebView(context).apply {
        ...
        if (isDebuggable) {
          Log.d("NexusWebView", "userAgent: ${settings.userAgentString}")
        }
        ...
        webViewClient =
          object : WebViewClient() {
            override fun onReceivedError(
              ...
              Log.e("NexusWebView", "onReceivedError: ${error.errorCode} ${error.description} ${request.url}")
            }
            ...
            override fun onPageFinished(view: WebView, url: String?) {
              if (isDebuggable) {
                Log.d("NexusWebView", "onPageFinished: $url")
              }
