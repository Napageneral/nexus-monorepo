...
import ai.nexus.android.LocationMode
import ai.nexus.android.MainViewModel
import ai.nexus.android.NodeForegroundService
...
fun SettingsSheet(viewModel: MainViewModel) {
  ...
  val smsPermissionLauncher =
    rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
      ...
      viewModel.refreshRuntimeConnection()
    }
  ...
  val visibleRuntimes =
    if (isConnected && remoteAddress != null) {
      runtimes.filterNot { "${it.host}:${it.port}" == remoteAddress }
    } else {
      runtimes
    }

  val runtimeDiscoveryFooterText =
    if (visibleRuntimes.isEmpty()) {
      discoveryStatusText
    } else if (isConnected) {
      "Discovery active • ${visibleRuntimes.size} other runtime${if (visibleRuntimes.size == 1) "" else "s"} found"
    } else {
      "Discovery active • ${visibleRuntimes.size} runtime${if (visibleRuntimes.size == 1) "" else "s"} found"
    }
  ...
  LazyColumn(
    ...
    // Order parity: Node → Runtime → Voice → Camera → Messaging → Location → Screen.
    ...
    // Runtime
    item { Text("Runtime", style = MaterialTheme.typography.titleSmall) }
    ...
    if (!isConnected || visibleRuntimes.isNotEmpty()) {
      item {
        Text(
          if (isConnected) "Other Runtimes" else "Discovered Runtimes",
          style = MaterialTheme.typography.titleSmall,
        )
      }
      if (!isConnected && visibleRuntimes.isEmpty()) {
        item { Text("No runtimes found yet.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
      } else {
        items(items = visibleRuntimes, key = { it.stableId }) { runtime ->
          val detailLines =
            buildList {
              add("IP: ${runtime.host}:${runtime.port}")
              runtime.lanHost?.let { add("LAN: $it") }
              runtime.tailnetDns?.let { add("Tailnet: $it") }
              if (runtime.runtimePort != null || runtime.canvasPort != null) {
                val gw = (runtime.runtimePort ?: runtime.port).toString()
                val canvas = runtime.canvasPort?.toString() ?: "—"
    ...
    item {
      ListItem(
        ...
        supportingContent = { Text("Manual runtime connection") },
        trailingContent = {
          Icon(
    ...
    item {
      AnimatedVisibility(visible = advancedExpanded) {
        Column(verticalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
          ListItem(
            headlineContent = { Text("Use Manual Runtime") },
          ...
          ListItem(
            ...
            supportingContent = { Text("Pin the runtime certificate on first connect.") },
    ...
    item {
      ListItem(
        ...
        supportingContent = { Text("Allows the runtime to request photos or short video clips (foreground only).") },
        ...
        supportingContent = {
          Text(
            if (smsPermissionAvailable) {
              "Allow the runtime to send SMS from this device."
            } else {
    ...
    item {
      Column(verticalArrangement = Arrangement.spacedBy(6.dp), modifier = Modifier.fillMaxWidth()) {
        ...
        ListItem(
          ...
          supportingContent = { Text("Only while Nexus is open.") },
          trailingContent = {
            RadioButton(
    ...
    item {
      ListItem(
        ...
        supportingContent = { Text("Keeps the screen awake while Nexus is open.") },
