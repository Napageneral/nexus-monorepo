...
@MainActor
struct NexusChatComposer: View {
    @Bindable var viewModel: NexusChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(NexusChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(NexusChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \NexusPendingAttachment.id)
                { (att: NexusPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            NexusPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NexusChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(NexusChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(NexusChatTheme.subtleCard)
        .clipShape(Capsule())
    }
