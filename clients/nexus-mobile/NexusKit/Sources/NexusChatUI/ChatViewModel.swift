...
private let chatUILogger = Logger(subsystem: "ai.nexus", category: "NexusChatUI")

@MainActor
@Observable
public final class NexusChatViewModel {
    public private(set) var messages: [NexusChatMessage] = []
    ...
    public var attachments: [NexusPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [NexusChatPendingToolCall] = []
    public private(set) var sessions: [NexusChatSessionEntry] = []
    private let transport: any NexusChatTransport
    ...
    private var pendingToolCallsById: [String: NexusChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any NexusChatTransport) {
    ...
    public var sessionChoices: [NexusChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [NexusChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [NexusChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: NexusPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [NexusChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: NexusChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [NexusChatMessage]) -> [NexusChatMessage] {
        var result: [NexusChatMessage] = []
    ...
    private static func dedupeKey(for message: NexusChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        guard self.healthOK else {
            self.errorText = "Runtime health not OK; cannot send"
            return
        }
        ...
        var userContent: [NexusChatMessageContent] = [
            NexusChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> NexusChatAttachmentPayload in
            NexusChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> NexusChatSessionEntry {
        NexusChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: NexusChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: NexusChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: NexusAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NexusChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> NexusPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
