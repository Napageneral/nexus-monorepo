...
public struct RuntimeTLSParams: Sendable {
...
public enum RuntimeTLSStore {
    private static let suiteName = "ai.nexus.shared"
    private static let keyPrefix = "runtime.tls."
...
public final class RuntimeTLSPinningSession: NSObject, WebSocketSessioning, URLSessionDelegate, @unchecked Sendable {
    private let params: RuntimeTLSParams
    ...
    public init(params: RuntimeTLSParams) {
        self.params = params
        super.init()
    }
        ...
        if let fingerprint = certificateFingerprint(trust) {
            ...
            if params.allowTOFU {
                if let storeKey = params.storeKey {
                    RuntimeTLSStore.saveFingerprint(fingerprint, stableID: storeKey)
                }
                completionHandler(.useCredential, URLCredential(trust: trust))
                return
            }
