...
public struct RuntimeConnectOptions: Sendable {
    ...
    // This is useful for secondary "operator" connections where the shared runtime token
...
public enum RuntimeAuthSource: String, Sendable {
...
private typealias ProtoAnyCodable = NexusProtocol.AnyCodable

private enum ConnectChallengeError: Error {
    case timeout
}

public actor RuntimeChannelActor {
    private let logger = Logger(subsystem: "ai.nexus", category: "runtime")
    private var task: WebSocketTaskBox?
    private var pending: [String: CheckedContinuation<RuntimeFrame, Error>] = [:]
    ...
    private var lastAuthSource: RuntimeAuthSource = .none
    ...
    private let pushHandler: (@Sendable (RuntimePush) async -> Void)?
    private let connectOptions: RuntimeConnectOptions?
    ...
    public init(
        ...
        pushHandler: (@Sendable (RuntimePush) async -> Void)? = nil,
        connectOptions: RuntimeConnectOptions? = nil,
    ...
    public func authSource() -> RuntimeAuthSource { self.lastAuthSource }
    ...
    public func shutdown() async {
        ...
        await self.failPending(NSError(
            domain: "Runtime",
            code: 0,
            userInfo: [NSLocalizedDescriptionKey: "runtime channel shutdown"]))
    ...
    private func watchdogLoop() async {
        ...
        while self.shouldReconnect {
            ...
            do {
                ...
                let wrapped = self.wrap(error, context: "runtime watchdog reconnect")
                self.logger.error("runtime watchdog reconnect failed \(wrapped.localizedDescription, privacy: .public)")
            }
        }
    }
    ...
    public func connect() async throws {
        ...
        do {
            ...
            let wrapped = self.wrap(error, context: "connect to runtime @ \(self.url.absoluteString)")
            self.connected = false
            self.task?.cancel(with: .goingAway, reason: nil)
            ...
            self.logger.error("runtime ws connect failed \(wrapped.localizedDescription, privacy: .public)")
            throw wrapped
        }
        self.listen()
        self.connected = true
    ...
    private func sendConnect() async throws {
        ...
        let options = self.connectOptions ?? RuntimeConnectOptions(
            role: "operator",
        ...
        var params: [String: ProtoAnyCodable] = [
            "minProtocol": ProtoAnyCodable(RUNTIME_PROTOCOL_VERSION),
            "maxProtocol": ProtoAnyCodable(RUNTIME_PROTOCOL_VERSION),
        ...
        // In that mode we always use the shared runtime token/password.
        let authToken = includeDeviceIdentity ? (storedToken ?? self.token) : self.token
        let authSource: RuntimeAuthSource
        if storedToken != nil {
        ...
        self.logger.info("runtime connect auth=\(authSource.rawValue, privacy: .public)")
    ...
    private func handleConnectResponse(
        ...
        if res.ok == false {
            let msg = (res.error?["message"]?.value as? String) ?? "runtime connect failed"
            throw NSError(domain: "Runtime", code: 1008, userInfo: [NSLocalizedDescriptionKey: msg])
        }
    ...
    private func handleReceiveFailure(_ err: Error) async {
        let wrapped = self.wrap(err, context: "runtime receive")
        self.logger.error("runtime ws receive failed \(wrapped.localizedDescription, privacy: .public)")
        self.connected = false
    ...
    private func handle(_ msg: URLSessionWebSocketTask.Message) async {
        ...
        guard let frame = try? self.decoder.decode(RuntimeFrame.self, from: data) else {
            self.logger.error("runtime decode failed")
            return
        }
        switch frame {
        case let .res(res):
    ...
    private func waitForConnectChallenge() async throws -> String? {
        ...
        do {
            return try await AsyncTimeout.withTimeout(
                ...
                operation: { [weak self] in
                    ...
                    while true {
                        ...
                        guard let frame = try? self.decoder.decode(RuntimeFrame.self, from: data) else { continue }
            ...
            if error is ConnectChallengeError {
                self.logger.warning("runtime connect challenge timed out")
                return nil
            }
            throw error
        }
    ...
    private func watchTicks() async {
        ...
        while self.connected {
            ...
            if let last = self.lastTick {
                ...
                if delta > tolerance {
                    self.logger.error("runtime tick missed; reconnecting")
                    self.connected = false
                    await self.failPending(
                        NSError(
                            domain: "Runtime",
                            code: 4,
                            userInfo: [NSLocalizedDescriptionKey: "runtime tick missed; reconnecting"]))
                    await self.scheduleReconnect()
                    return
                }
            }
    ...
    private func scheduleReconnect() async {
        ...
        do {
            ...
            let wrapped = self.wrap(error, context: "runtime reconnect")
            self.logger.error("runtime reconnect failed \(wrapped.localizedDescription, privacy: .public)")
            await self.scheduleReconnect()
        }
    }
    ...
    {
        try await self.connectOrThrow(context: "runtime connect")
        ...
        let response = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<RuntimeFrame, Error>) in
            ...
            Task {
                do {
                    ...
                    let wrapped = self.wrap(error, context: "runtime send \(method)")
        ...
        guard case let .res(res) = response else {
            throw NSError(domain: "Runtime", code: 2, userInfo: [NSLocalizedDescriptionKey: "unexpected frame"])
        }
        if res.ok == false {
            ...
            throw RuntimeResponseError(method: method, code: code, message: msg, details: details)
        }
    ...
    public func send(method: String, params: [String: AnyCodable]?) async throws {
        ...
        guard let task = self.task else {
            throw NSError(
                domain: "Runtime",
                code: 5,
                userInfo: [NSLocalizedDescriptionKey: "runtime socket unavailable"])
        }
        do {
    ...
    public func sendEvent(event: String, payload: [String: AnyCodable]?) async throws {
        try await self.connectOrThrow(context: "runtime connect")
        let data = try self.encodeEvent(event: event, payload: payload)
        guard let task = self.task else {
            throw NSError(
                domain: "Runtime",
                code: 5,
        ...
        do {
            ...
            let wrapped = self.wrap(error, context: "runtime event \(event)")
            self.connected = false
            self.task?.cancel(with: .goingAway, reason: nil)
            Task { [weak self] in
                guard let self else { return }
        ...
        kind: String) throws -> (id: String, data: Data)
    {
        ...
        do {
            ...
            self.logger.error(
                "runtime \(kind) encode failed \(method, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
            throw error
        }
    }

    private func encodeEvent(event: String, payload: [String: AnyCodable]?) throws -> Data {
        let payloadObject: ProtoAnyCodable? = payload.map { entries in
            let dict = entries.reduce(into: [String: ProtoAnyCodable]()) { dict, entry in
                dict[entry.key] = ProtoAnyCodable(entry.value.value)
            }
            return ProtoAnyCodable(dict)
        ...
        let frame = EventFrame(
            ...
            stateversion: nil)
        do {
            return try self.encoder.encode(frame)
        } catch {
            self.logger.error(
                "runtime event encode failed \(event, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
            throw error
        }
    }
    ...
    private func timeoutRequest(id: String, timeoutMs: Double) async {
        ...
        let err = NSError(
            domain: "Runtime",
            code: 5,
            userInfo: [NSLocalizedDescriptionKey: "runtime request timed out after \(Int(timeoutMs))ms"])
        waiter.resume(throwing: err)
    }
}

// Intentionally no `RuntimeChannel` wrapper: the app should use the single shared `RuntimeConnection`.
