...
private final class TestChatTransport: @unchecked Sendable, NexusChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NexusChatHistoryPayload]
    private let sessionsResponses: [NexusChatSessionsListResponse]

    private let stream: AsyncStream<NexusChatTransportEvent>
    private let continuation: AsyncStream<NexusChatTransportEvent>.Continuation

    init(
        historyResponses: [NexusChatHistoryPayload],
        sessionsResponses: [NexusChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<NexusChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> NexusChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NexusChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [NexusChatAttachmentPayload]) async throws -> NexusChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return NexusChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> NexusChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? NexusChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: NexusChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = NexusChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = NexusChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { NexusChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = NexusChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = NexusChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = NexusChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { NexusChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
    ...
    @Test func abortRequestsDoNotClearPendingUntilAbortedEvent() async throws {
        ...
        // Pending remains until the runtime broadcasts an aborted/final chat event.
