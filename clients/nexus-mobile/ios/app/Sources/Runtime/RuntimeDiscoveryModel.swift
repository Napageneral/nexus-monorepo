...
@MainActor
@Observable
final class RuntimeDiscoveryModel {
    ...
    struct DiscoveredRuntime: Identifiable, Equatable {
    ...
    var runtimes: [DiscoveredRuntime] = []
    var statusText: String = "Idle"
    private(set) var debugLog: [DebugLogEntry] = []

    private var browsers: [String: NWBrowser] = [:]
    private var runtimesByDomain: [String: [DiscoveredRuntime]] = [:]
    ...
    func setDebugLoggingEnabled(_ enabled: Bool) {
        ...
        if !enabled {
            ...
            self.appendDebugLog("snapshot: status=\(self.statusText) runtimes=\(self.runtimes.count)")
        }
    }
    ...
    func start() {
        ...
        for domain in NexusBonjour.runtimeServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: NexusBonjour.runtimeServiceType, domain: domain),
                using: params)
            ...
            browser.browseResultsChangedHandler = { [weak self] results, _ in
                Task { @MainActor in
                    guard let self else { return }
                    self.runtimesByDomain[domain] = results.compactMap { result -> DiscoveredRuntime? in
                        ...
                        case let .service(name, _, _, _):
                            ...
                            return DiscoveredRuntime(
                                ...
                                stableID: RuntimeEndpointID.stableID(result.endpoint),
                                debugID: RuntimeEndpointID.prettyDescription(result.endpoint),
                                lanHost: Self.txtValue(txt, key: "lanHost"),
                                tailnetDns: Self.txtValue(txt, key: "tailnetDns"),
                                runtimePort: Self.txtIntValue(txt, key: "runtimePort"),
                                canvasPort: Self.txtIntValue(txt, key: "canvasPort"),
                                tlsEnabled: Self.txtBoolValue(txt, key: "runtimeTls"),
                                tlsFingerprintSha256: Self.txtValue(txt, key: "runtimeTlsSha256"),
                    ...
                    self.recomputeRuntimes()
                }
            }

            self.browsers[domain] = browser
            browser.start(queue: DispatchQueue(label: "bot.molt.ios.runtime-discovery.\(domain)"))
        }
    }
    ...
    func stop() {
        ...
        self.browsers = [:]
        self.runtimesByDomain = [:]
        self.statesByDomain = [:]
        self.runtimes = []
        self.statusText = "Stopped"
    }

    private func recomputeRuntimes() {
        let next = self.runtimesByDomain.values
            .flatMap(\.self)
            .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
    ...
    private static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Nexus)", with: "")
