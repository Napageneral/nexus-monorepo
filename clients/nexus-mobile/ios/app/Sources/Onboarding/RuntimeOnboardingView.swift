...
struct RuntimeOnboardingView: View {
    var body: some View {
        NavigationStack {
            List {
                Section {
                    Text("Connect to your runtime to get started.")
            ...
            .navigationTitle("Connect Runtime")
        }
    }
}
...
private struct AutoDetectStep: View {
    ...
    var body: some View {
        Form {
            Section {
                Text("We’ll scan for runtimes on your network and connect automatically when we find one.")
                    .foregroundStyle(.secondary)
            }
        ...
        .onChange(of: self.runtimeController.runtimes) { _, _ in
            self.triggerAutoConnect()
        }
    }

    private func triggerAutoConnect() {
        guard self.appModel.runtimeServerName == nil else { return }
        guard self.connectingRuntimeID == nil else { return }
        guard let candidate = self.autoCandidate() else { return }

        self.connectingRuntimeID = candidate.id
        Task {
            defer { self.connectingRuntimeID = nil }
            await self.runtimeController.connect(candidate)
        }
    }

    private func autoCandidate() -> RuntimeDiscoveryModel.DiscoveredRuntime? {
        let preferred = self.preferredRuntimeStableID.trimmingCharacters(in: .whitespacesAndNewlines)
        let lastDiscovered = self.lastDiscoveredRuntimeStableID.trimmingCharacters(in: .whitespacesAndNewlines)

        if !preferred.isEmpty,
           let match = self.runtimeController.runtimes.first(where: { $0.stableID == preferred })
        {
            return match
        }
        if !lastDiscovered.isEmpty,
           let match = self.runtimeController.runtimes.first(where: { $0.stableID == lastDiscovered })
        {
            return match
        }
        if self.runtimeController.runtimes.count == 1 {
            return self.runtimeController.runtimes.first
        }
        return nil
    }

    private func connectionStatusLines() -> [String] {
        ConnectionStatusBox.defaultLines(appModel: self.appModel, runtimeController: self.runtimeController)
    }

    private func resetConnectionState() {
        self.appModel.disconnectRuntime()
        self.connectStatusText = nil
        self.connectingRuntimeID = nil
    }
}
...
private struct ManualEntryStep: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                TextField("Runtime token", text: self.$manualToken)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()

                SecureField("Runtime password", text: self.$manualPassword)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
            }
    ...
    private func connectManual() async {
        ...
        defaults.set(true, forKey: "runtime.manual.enabled")
        defaults.set(host, forKey: "runtime.manual.host")
        defaults.set(self.manualPortValue() ?? 0, forKey: "runtime.manual.port")
        defaults.set(self.manualUseTLS, forKey: "runtime.manual.tls")
           ...
           !instanceId.isEmpty
        {
            ...
            if !trimmedToken.isEmpty {
                RuntimeSettingsStore.saveRuntimeToken(trimmedToken, instanceId: instanceId)
            }
            RuntimeSettingsStore.saveRuntimePassword(trimmedPassword, instanceId: instanceId)
        }
...
private struct ConnectionStatusBox: View {
    ...
    static func defaultLines(
        ...
        runtimeController: RuntimeConnectionController
    ) -> [String] {
        var lines: [String] = [
            "runtime: \(appModel.runtimeStatusText)",
            "discovery: \(runtimeController.discoveryStatusText)",
        ]
        lines.append("server: \(appModel.runtimeServerName ?? "—")")
        lines.append("address: \(appModel.runtimeRemoteAddress ?? "—")")
        return lines
    }
}
