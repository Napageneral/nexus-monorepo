...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> NexusNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> NexusNetworkStatusPayload {
        let status: NexusNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> NexusNetworkStatusPayload {
        NexusNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
