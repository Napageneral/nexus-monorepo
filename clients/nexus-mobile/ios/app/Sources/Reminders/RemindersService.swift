...
final class RemindersService: RemindersServicing {
    func list(params: NexusRemindersListParams) async throws -> NexusRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NexusReminderPayload], Error>) in
        ...
        return NexusRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: NexusRemindersAddParams) async throws -> NexusRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = NexusReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return NexusRemindersAddPayload(reminder: payload)
    }
