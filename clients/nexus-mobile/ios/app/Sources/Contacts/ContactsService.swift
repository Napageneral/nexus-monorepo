...
final class ContactsService: ContactsServicing {
    ...
    func search(params: NexusContactsSearchParams) async throws -> NexusContactsSearchPayload {
        ...
        return NexusContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: NexusContactsAddParams) async throws -> NexusContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return NexusContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return NexusContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> NexusContactPayload {
        NexusContactPayload(
