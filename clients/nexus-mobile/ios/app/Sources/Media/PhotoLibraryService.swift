...
final class PhotoLibraryService: PhotosServicing {
    // The runtime WebSocket has a max payload size; returning large base64 blobs
    // can cause the runtime to close the connection. Keep photo payloads small
    ...
    func latest(params: NexusPhotosLatestParams) async throws -> NexusPhotosLatestPayload {
        ...
        assets.enumerateObjects { asset, _, stop in
            ...
            if let payload = try? Self.renderAsset(
                ...
                formatter: formatter)
            {
                // Keep the entire response under the runtime WS max payload.
        ...
        return NexusPhotosLatestPayload(photos: results)
    }
    ...
    private static func renderAsset(
        ...
        maxWidth: Int,
        quality: Double,
        formatter: ISO8601DateFormatter) throws -> NexusPhotoPayload
    {
        ...
        maxBase64Chars: Int) throws -> (Data, UIImage)
    {
        ...
        throw NSError(domain: "Photos", code: 4, userInfo: [
            NSLocalizedDescriptionKey: "photo too large for runtime transport; try smaller maxWidth/quality",
        ])
    }
