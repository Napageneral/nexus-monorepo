...
final class MotionService: MotionServicing {
    func activities(params: NexusMotionActivityParams) async throws -> NexusMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NexusMotionActivityEntry], Error>) in
        ...
        return NexusMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: NexusPedometerParams) async throws -> NexusPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<NexusPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = NexusPedometerPayload(
