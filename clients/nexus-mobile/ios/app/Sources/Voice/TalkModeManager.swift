...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<NexusTalkPTTStopPayload, Never>?
    ...
    private var runtime: RuntimeNodeSession?
    ...
    func attachRuntime(_ runtime: RuntimeNodeSession) {
        self.runtime = runtime
    }

    func updateRuntimeConnected(_ connected: Bool) {
        self.runtimeConnected = connected
        if connected {
            // If talk mode is enabled before the runtime connects (common on cold start),
    ...
    func updateMainSessionKey(_ sessionKey: String?) {
        ...
        if self.runtimeConnected, self.isEnabled {
    ...
    func start() async {
        ...
        guard self.runtimeConnected else {
            self.statusText = "Offline"
            return
        }
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = NexusTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> NexusTalkPTTStartPayload {
        guard self.runtimeConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return NexusTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> NexusTalkPTTStopPayload {
        ...
        guard self.runtimeConnected else {
            self.statusText = "Runtime not connected"
            if self.resumeContinuousAfterPTT {
                await self.start()
            }
            self.resumeContinuousAfterPTT = false
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> NexusTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return NexusTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> NexusTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = NexusTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func startRecognition() throws {
        ...
        RuntimeDiagnostics.log("talk audio: session \(Self.describeAudioSession())")
        ...
        self.recognitionTask = recognizer.recognitionTask(with: request) { [weak self] result, error in
            ...
            if let error {
                ...
                RuntimeDiagnostics.log("talk speech: error=\(msg)")
                if !self.isSpeaking {
            ...
            if !result.isFinal, !self.loggedPartialThisCycle {
                ...
                if !trimmed.isEmpty {
                    ...
                    RuntimeDiagnostics.log("talk speech: partial chars=\(trimmed.count)")
                }
            }
            Task { @MainActor in
    ...
    private func restartRecognitionAfterError() async {
        ...
        do {
            ...
            RuntimeDiagnostics.log("talk speech: recognition restarted")
        } catch {
            let msg = error.localizedDescription
            RuntimeDiagnostics.log("talk speech: restart failed error=\(msg)")
        }
    }
    ...
    private func handleTranscript(transcript: String, isFinal: Bool) async {
        ...
        if isFinal {
            self.lastTranscript = trimmed
            guard !trimmed.isEmpty else { return }
            RuntimeDiagnostics.log("talk speech: final transcript chars=\(trimmed.count)")
            self.loggedPartialThisCycle = false
    ...
    private func finishPTTOnce(_ payload: NexusTalkPTTStopPayload) {
    ...
    private func processTranscript(_ transcript: String, restartAfter: Bool) async {
        ...
        RuntimeDiagnostics.log("talk: process transcript chars=\(transcript.count) restartAfter=\(restartAfter)")
        await self.reloadConfig()
        let prompt = self.buildPrompt(transcript: transcript)
        guard self.runtimeConnected, let runtime else {
            self.statusText = "Runtime not connected"
            self.logger.warning("finalize: runtime not connected")
            RuntimeDiagnostics.log("talk: abort runtime not connected")
            if restartAfter {
                await self.start()
            }
            return
        ...
        do {
            ...
            RuntimeDiagnostics.log("talk: chat.send start sessionKey=\(sessionKey) chars=\(prompt.count)")
            let runId = try await self.sendChat(prompt, runtime: runtime)
            self.logger.info("chat.send ok runId=\(runId, privacy: .public)")
            RuntimeDiagnostics.log("talk: chat.send ok runId=\(runId)")
            ...
            if shouldIncremental {
                ...
                streamingTask = Task { @MainActor [weak self] in
                    guard let self else { return }
                    await self.streamAssistant(runId: runId, runtime: runtime)
                }
            }
            let completion = await self.waitForChatCompletion(runId: runId, runtime: runtime, timeoutSeconds: 120)
            if completion == .timeout {
                self.logger.warning(
                    "chat completion timeout runId=\(runId, privacy: .public); attempting history fallback")
                RuntimeDiagnostics.log("talk: chat completion timeout runId=\(runId)")
            } else if completion == .aborted {
                self.statusText = "Aborted"
                self.logger.warning("chat completion aborted runId=\(runId, privacy: .public)")
                RuntimeDiagnostics.log("talk: chat completion aborted runId=\(runId)")
                ...
                RuntimeDiagnostics.log("talk: chat completion error runId=\(runId)")
                streamingTask?.cancel()
                await self.finishIncrementalSpeech()
                await self.start()
                return
            ...
            guard let assistantText else {
                ...
                RuntimeDiagnostics.log("talk: assistant text timeout runId=\(runId)")
                streamingTask?.cancel()
                await self.finishIncrementalSpeech()
                await self.start()
                return
            ...
            RuntimeDiagnostics.log("talk: assistant text ok chars=\(assistantText.count)")
            streamingTask?.cancel()
            if shouldIncremental {
            ...
            RuntimeDiagnostics.log("talk: failed error=\(error.localizedDescription)")
        }
    ...
    private func subscribeChatIfNeeded(sessionKey: String) async {
        ...
        self.logger.info("chat session active sessionKey=\(key, privacy: .public)")
    }
    ...
    private func sendChat(_ message: String, runtime: RuntimeNodeSession) async throws -> String {
        ...
        let res = try await runtime.request(method: "chat.send", paramsJSON: json, timeoutSeconds: 30)
    ...
    private func waitForChatCompletion(
        runId: String,
        runtime: RuntimeNodeSession,
        timeoutSeconds: Int = 120) async -> ChatCompletionState
    {
        let stream = await runtime.subscribeServerEvents(bufferingNewest: 200)
        return await withTaskGroup(of: ChatCompletionState.self) { group in
            group.addTask { [runId] in
                for await evt in stream {
                    if Task.isCancelled { return .timeout }
                    guard evt.event == "chat", let payload = evt.payload else { continue }
                    guard let chatEvent = try? RuntimePayloadDecoding.decode(payload, as: ChatEvent.self) else {
                        continue
                    }
        ...
        timeoutSeconds: Int) async throws -> String?
    {
        ...
        while Date() < deadline {
            if let text = try await self.fetchLatestAssistantText(runtime: runtime, since: since) {
                return text
            }
    ...
    private func fetchLatestAssistantText(runtime: RuntimeNodeSession, since: Double? = nil) async throws -> String? {
        let res = try await runtime.request(
            method: "chat.history",
    ...
    private func playAssistant(text: String) async {
        ...
        do {
            ...
            if canUseElevenLabs, let voiceId, let apiKey {
                RuntimeDiagnostics.log("talk tts: provider=elevenlabs voiceId=\(voiceId)")
                ...
                if let modelId {
                    RuntimeDiagnostics.log("talk tts: modelId=\(modelId)")
                }
                ...
                RuntimeDiagnostics.log("talk tts: provider=system (missing key or voiceId)")
                if self.interruptOnSpeech {
                    do {
                        try self.startRecognition()
                    } catch {
            ...
            RuntimeDiagnostics.log("talk tts: provider=system (error) msg=\(error.localizedDescription)")
            do {
                if self.interruptOnSpeech {
                    do {
                        try self.startRecognition()
    ...
    private func streamAssistant(runId: String, runtime: RuntimeNodeSession) async {
        let stream = await runtime.subscribeServerEvents(bufferingNewest: 200)
        for await evt in stream {
            if Task.isCancelled { return }
            guard evt.event == "agent", let payload = evt.payload else { continue }
            guard let agentEvent = try? RuntimePayloadDecoding.decode(payload, as: NexusAgentEventPayload.self) else {
                continue
            }
...
extension TalkModeManager {
    ...
    func reloadConfig() async {
        ...
        do {
            let res = try await runtime.request(method: "config.get", paramsJSON: "{}", timeoutSeconds: 8)
