...
module.exports = async function handler(req, res) {
  ...
  const parsed = new URL(req.url || "/api/workspaces/provisioning/status", "http://localhost");
  const requestId = (parsed.searchParams.get("request_id") || "").trim();
  ...
  try {
    const proxied = await proxyToFrontdoor({
      ...
      path: `/api/workspaces/provisioning/status${query}`,
      method: "GET",
